"""Compare top Optuna trials with 5 seeds on Fold-5.

Trials to test (top by Fold-5 nMAE, excluding the high-LR outlier):
- trial 37: lr=0.02669, leaves=65, mdl=27  (f5=7.5558 with 3 seeds)
- trial 66: lr=0.00987, leaves=50, mdl=18  (f5=7.5606 with 3 seeds)
- trial 23: lr=0.02595, leaves=55, mdl=43  (f5=7.5607 with 3 seeds)
"""
...
# Load study to get trial params
study = pickle.load(open(str(_ROOT / 'models' / 'optuna_v27_cf_study.pkl'), 'rb'))
trial_nums = [37, 66, 23]
trial_params = {t.number: t.params for t in study.trials if t.number in trial_nums}
...
# Probe once with a stable config to get top-K
probe_cfg = LGBMConfig(num_leaves=65, min_data_in_leaf=27, learning_rate=0.02669,
                       feature_fraction=0.5, bagging_fraction=0.6, bagging_freq=5,
                       lambda_l1=0.1, lambda_l2=0.01, num_boost_round=5000,
                       early_stopping_rounds=250, log_period=0)
...
probe = lgb.train(probe_cfg.to_params(), dt, num_boost_round=5000,
...
for trial_num in trial_nums:
    ...
    print(f'\nTrial {trial_num}: lr={params["learning_rate"]:.5f} leaves={params["num_leaves"]}')
    regime_preds = {}
    for name, (lo, hi) in [('low_0_7', (0, 7)), ('mid_4_12', (4, 12)), ('high_8_25', (8, 25))]:
        ...
        for s in SEEDS:
            ...
            b = lgb.train(cfg.to_params(), dt2, num_boost_round=5000,
        ...
        print(f'  {name}: {normalized_mae(y_va_mw, preds_mw):.4f}%  iters={iters}')
    ...
    results[trial_num] = nmae
    print(f'  => Fold-5: {nmae:.4f}%')

print('\n=== Summary ===')
for t, n in sorted(results.items(), key=lambda x: x[1]):
    print(f'  trial {t}: {n:.4f}%')
