"""V27: Raw MW target (no CF transform) + 3-fold CV-bag.

The CF transform gave -0.12 pp on Fold-5 (v16 vs v15), but we never verified
it helps on LB. Given that every Fold-5 improvement has hurt LB, maybe the
simpler raw MW target is actually better for generalization.

This is the v15 regime specialist approach (raw MW target, clip to [0, 90.09])
but with the v20 CV-bag methodology (3 folds, early stopping per fold).

Usage:
    python scripts/make_v27_raw_mw_cvbag.py
"""
...
def main():
    set_global_seed(42)
    print("=" * 60)
    print("V27: Raw MW target + 3-fold CV-bag (no CF transform)")
    print("=" * 60)
    ...
    # Feature probe from Fold-5 (using raw MW target).
    fold5 = folds[-1]
    ...
    y_tr_mw_ = df_t_[TARGET_COL].to_numpy(dtype=np.float32)
    y_va_mw_ = df_v_[TARGET_COL].to_numpy(dtype=np.float32)
    ...
    dt_ = lgb.Dataset(X_t_all_, label=y_tr_mw_, feature_name=feat_cols_all, free_raw_data=False)
    dv_ = lgb.Dataset(X_v_all_, label=y_va_mw_, feature_name=feat_cols_all, free_raw_data=False)
    ...
    print(f"  Selected {len(top_k)} features (MW-based importance)")

    # 3-fold CV-bag with raw MW target.
    test_preds_per_fold = []
    for fold_idx in FOLD_IDS:
        ...
        for name, (lo, hi) in [("low", (0, 7)), ("mid", (4, 12)), ("high", (8, 25))]:
            ...
            for s in SEEDS_SPEC:
                ...
                dt = lgb.Dataset(X_tr, label=y_tr_mw, weight=weights, feature_name=top_k, free_raw_data=False)
                dv = lgb.Dataset(X_va, label=y_va_mw, feature_name=top_k, free_raw_data=False)
                b = lgb.train(cfg.to_params(), dt, num_boost_round=5000,
                              valid_sets=[dv], valid_names=["val"],
                              callbacks=[lgb.early_stopping(250, verbose=False)])
                test_preds.append(np.clip(b.predict(X_test, num_iteration=b.best_iteration), 0, CAPACITY_MW))
                val_preds.append(np.clip(b.predict(X_va, num_iteration=b.best_iteration), 0, CAPACITY_MW))
            regime_test_preds[name] = np.mean(test_preds, axis=0)
            regime_val_preds[name] = np.mean(val_preds, axis=0)

        fold_test_mw = np.mean(list(regime_test_preds.values()), axis=0)
        fold_val_mw = np.mean(list(regime_val_preds.values()), axis=0)
        test_preds_per_fold.append(fold_test_mw)

        fold_nmae = normalized_mae(y_va_mw, fold_val_mw)
        print(f"  Fold {fold_idx+1} val nMAE: {fold_nmae:.4f}%")

    # Average test predictions across folds.
    test_mw = np.clip(np.mean(test_preds_per_fold, axis=0), 0, CAPACITY_MW)

    order = df_valid_sorted["_submission_row"].to_numpy().astype(int)
    po = np.empty_like(test_mw)
    po[order] = test_mw

    out_path = _ROOT / "submissions" / "archive" / "v27.0_raw_mw_cv3.csv"
    write_submission(po, out_path, expected_rows=len(df_valid))
    print(f"\nSaved: {out_path.name}  mean={test_mw.mean():.2f} MW")

    # Blend with v20.1 (CF target).
    ...
    if v20_path.exists():
        ...
        blend = 0.5 * po + 0.5 * v20_preds
        blend_path = _ROOT / "submissions" / "archive" / "v27.1_mw50_cf50.csv"
        write_submission(blend, blend_path, expected_rows=len(df_valid), timestamps=ts)
        print(f"  Blend MW50/CF50: mean={blend.mean():.2f}")
