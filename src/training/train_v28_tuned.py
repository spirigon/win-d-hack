"""V28: CF-target regime-specialist ensemble with Optuna-tuned HPs.

Differences from v15/v16:
- Hyperparameters from tune_v27_cf.py (100 Optuna trials on CF target).
- K=70 confirmed optimal by K-sweep.
- CF target throughout (same as v16 but with re-tuned HPs).
- 5 seeds per specialist (same as v15).

Expected Fold-5: ~7.52% (Optuna used 3 seeds; 5-seed ensemble should be
slightly better due to variance reduction).

Usage:
    python -m src.training.train_v28_tuned
"""
...
SUBMISSION_PATH = _ROOT / "submissions" / "archive" / "v28.0_tuned_cf.csv"
...
# Optuna-tuned HPs from tune_v27_cf.py (100 trials, CF target, K=70).
# Fold-5 (3-seed): 7.5281%  Fold-4: 7.0820%
CONFIG = LGBMConfig(
    num_leaves=61,
    min_data_in_leaf=21,
    ...
    num_boost_round=3000,  # match Optuna tuning budget (high LR converges fast)
...
def _train_specialists_valid(X_tr, y_tr_cf, X_va, y_va_cf, feat_cols, ws_tr, sample_weight):
    """Train 3 regime specialists with early stopping. Return (preds_mw, best_iters)."""
    regime_preds = {}
    regime_iters = {}
...
def _train_specialists_full(X, y_cf, feat_cols, ws, sample_weight, n_rounds_per_spec, X_test):
    """Train specialists on full data (no early stopping). Return test predictions."""
    regime_preds = {}
    for name, (lo, hi) in [("low_0_7", (0, 7)), ("mid_4_12", (4, 12)), ("high_8_25", (8, 25))]:
        mask_in = (ws >= lo) & (ws < hi)
        regime_w = np.where(mask_in, 2.0, 0.3).astype(np.float32)
...
def main() -> None:
    set_global_seed(42)
    print("V28: CF-target + Optuna-tuned HPs")
    print("=" * 60)
    ...
    # Determine rounds: median best_iter × 1.2 per specialist.
    n_rounds_per_spec = {}
    for name, iters in regime_iters.items():
        n_rounds_per_spec[name] = max(int(np.median(iters) * 1.2), 2000)
