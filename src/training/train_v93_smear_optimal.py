"""V93: V92 + smeared power at optimal delta=2.175 m/s, K=80, 5 seeds, GPU.

Motivation: empirical analysis found delta=2.175 m/s minimises the smear
residual (best captures NWP forecast uncertainty in the power-curve nonlinearity).
Previous runs used delta=[1,2,3,4] and got 0/K — hypothesis is that the spread
of 4 deltas dilutes the GAIN signal; a single tuned delta may be distinct enough
to survive the probe.

New vs V92:
  smear_p2175_per_turb_kw  — (P(v-2.175) + P(v+2.175)) / 2  per turbine (kW)
  smear_p2175_farm_mw      — farm-level smeared power (MW)
  smear_p2175_excess_mw    — smear_farm - standard P(v) farm (MW)

K=80, 5 seeds, GPU-accelerated (RTX 3060 Ti), CF+MW blend.

Outputs:
    data/processed/v93_oof.parquet
    submissions/archive/v93.0_smear_optimal.csv
"""
...
import src.features.smeared_power_features as _smear_mod
from src.features.smeared_power_features import add_smeared_power_features, smeared_power_columns
...
# Override smear delta to optimal single value
_smear_mod.DELTA_V_LIST = [2.175]

TRAIN_PATH  = _ROOT / "data" / "raw" / "train_dataset.csv"
VALID_PATH  = _ROOT / "data" / "raw" / "valid_features.csv"
ERA5_PATH   = _ROOT / "data" / "external" / "era5_reanalysis.parquet"
OOF_PATH    = _ROOT / "data" / "processed" / "v93_oof.parquet"
OUTPUT_PATH = _ROOT / "submissions" / "archive" / "v93.0_smear_optimal.csv"

MARCH_WEIGHT = 3.0
SEEDS = SEEDS_5

_GPU_OVERRIDES: dict = {}  # GPU causes silent crashes on WDDM during multi-model fold training
...
def main() -> None:
    ...
    print("=" * 72)
    print(f"V93: V92 + smear(delta=2.175 m/s)  ({len(SEEDS)} seeds, K={K})")
    print("     hub-height 85m + seasonal + optimal smear, GPU")
    print("=" * 72)
    ...
    combined = add_smeared_power_features(combined)   # δ=2.175 only
    ...
    print(f"  Feature groups: nwp={len(nwp_cols)} nasa={len(nasa_cols)} "
          ...
          f"hub={len(hub_cols)} seasonal={len(seas_cols)} smear={len(smear_cols)}")
    ...
    all_extra_cols = list(dict.fromkeys(
        nwp_cols + nasa_cols + mnwp_cols + adv_cols + hub_cols + seas_cols + smear_cols
    ))
    feat_cols_all = [
    ...
    for label, cols in [("hub", hub_cols), ("seasonal", seas_cols), ("smear", smear_cols)]:
