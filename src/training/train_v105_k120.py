"""V105: V99 pipeline with K=120 probe instead of K=90.

At K=90 we select top-90 from 312 post-dedup features (29% of pool).
K=120 allows 33% more features, potentially recovering signal from
features that rank 91-120 but have real predictive power (e.g., hub_height
features that were in top-80 at v92 but fell below 90 after pool expansion).

All other hyperparameters identical to V99: corr dedup (|r|>0.999),
GEM/ICON-G features, 5 seeds, CF+MW blend.

Outputs:
    data/processed/v105_oof.parquet
    submissions/archive/v105.0_k120.csv
"""
...
OOF_PATH    = _ROOT / "data" / "processed" / "v105_oof.parquet"
OUTPUT_PATH = _ROOT / "submissions" / "archive" / "v105.0_k120.csv"

MARCH_WEIGHT   = 3.0
BLEND_WEIGHT_MW = 0.0  # CF-only proven optimal by blend_weight_optimizer
SEEDS          = SEEDS_5
K              = 120  # increased from 90
...
def main() -> None:
    ...
    print("=" * 72)
    print(f"V105: K=120 probe + corr dedup + GEM/ICON-G  ({len(SEEDS)} seeds, K={K})")
    print("=" * 72)
    ...
    # Report rank-90 and rank-120 GAIN to see if the extra features have real signal
    if len(feat_imp) >= K:
        rank90_gain = feat_imp[89][1] if len(feat_imp) > 89 else 0
        rank120_gain = feat_imp[K-1][1] if len(feat_imp) >= K else 0
        print(f"  GAIN at rank-90: {rank90_gain:.1f}  |  rank-{K}: {rank120_gain:.1f}")
    for label, cols in [
