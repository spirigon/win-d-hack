"""V100: V99 + NWP-ERA5 wind direction disagreement features.

The main NWP (hackathon weather model) and ERA5 reanalysis can disagree on
wind direction even when they agree on wind speed.  Direction disagreement
captures a qualitatively different uncertainty signal — a key missing feature.

Features added:
    nwp_era5_dir_agreement  : dot product of NWP 120m vs ERA5 100m unit vectors
                              (+1 = perfect agreement, -1 = 180° opposite)
    nwp_era5_dir_diff_deg   : absolute direction disagreement in degrees (0-180)
    nwp_era5_dir_signed_diff: signed difference degrees (NWP − ERA5, −180 to +180)

Note: NWP direction in raw data is degrees/1000 (so 0.276 = 276°).
      ERA5 direction is in raw degrees (245 = 245°).

K=90, corr dedup, GEM/ICON-G, 5 seeds, CPU.

Outputs:
    data/processed/v100_oof.parquet
    submissions/archive/v100.0_dir_disagree.csv
"""
...
OOF_PATH    = _ROOT / "data" / "processed" / "v100_oof.parquet"
OUTPUT_PATH = _ROOT / "submissions" / "archive" / "v100.0_dir_disagree.csv"
...
def _add_dir_disagreement(df: pd.DataFrame) -> pd.DataFrame:
    """NWP (120m, degrees/1000) vs ERA5 (100m, raw degrees) direction disagreement."""
    if "wind_direction_120m" not in df.columns or "era5_wind_direction_100m" not in df.columns:
        return df
    df = df.copy()
    nwp_rad  = np.deg2rad(df["wind_direction_120m"].to_numpy(dtype=np.float64) * 1000.0)
    era5_rad = np.deg2rad(df["era5_wind_direction_100m"].to_numpy(dtype=np.float64))
    # Cosine similarity (dot product of unit vectors)
    dot = np.cos(nwp_rad) * np.cos(era5_rad) + np.sin(nwp_rad) * np.sin(era5_rad)
    dot = np.clip(dot, -1.0, 1.0)
    df["nwp_era5_dir_agreement"]  = dot.astype(np.float32)
    df["nwp_era5_dir_diff_deg"]   = np.rad2deg(np.arccos(dot)).astype(np.float32)
    # Signed difference: NWP − ERA5, wrapped to (−180, 180]
    nwp_deg  = df["wind_direction_120m"].to_numpy(dtype=np.float64) * 1000.0
    era5_deg = df["era5_wind_direction_100m"].to_numpy(dtype=np.float64)
    signed   = ((nwp_deg - era5_deg) + 180.0) % 360.0 - 180.0
    df["nwp_era5_dir_signed_diff"] = signed.astype(np.float32)
    return df
...
def main() -> None:
    ...
    print("=" * 72)
    print(f"V100: dir disagree + corr dedup + GEM/ICON-G  ({len(SEEDS)} seeds, K={K})")
    print("=" * 72)
    ...
    combined = _add_dir_disagreement(combined)   # NEW: NWP-ERA5 direction disagreement
    ...
    dir_cols  = ["nwp_era5_dir_agreement", "nwp_era5_dir_diff_deg", "nwp_era5_dir_signed_diff"]
    dir_cols  = [c for c in dir_cols if c in combined.columns]
    ...
    all_extra_cols = list(dict.fromkeys(
        nwp_cols + nasa_cols + mnwp_cols + adv_cols + hub_cols + seas_cols + gem_cols + dir_cols
    ))
    feat_cols_all = [
    ...
    for label, cols in [
        ...
        ("gem_icon", gem_cols), ("dir_disagree", dir_cols),
    ]:
