"""V34: V32 + walk-forward seasonal-availability features + iso-recal.

Same v27.1 mw50/cf50 architecture, ERA5v2 features, and hyperparameters as
``train_v32_era5v2``, plus:

  1. Walk-forward (month, hour) seasonal availability rate computed on
     past-only data — ``src.features.availability``. No target leakage.
  2. Per-leg isotonic re-calibration on OOF predictions — corrects the
     ``+1.32 MW`` rated-band bias surfaced in the v32 Fold-5 diagnostic.

The script saves OOF predictions and per-fold test predictions for
downstream ridge stacking, and writes a final blended + iso-recalibrated
submission.

Outputs:

    data/processed/v34_oof.parquet
    data/processed/v34_test.parquet
    submissions/archive/v34.0_curtailment_iso.csv

Usage:

    python -m src.training.train_v34_curtailment --seeds 5
"""
...
from src.postprocess.iso_recal import (
    apply_iso_recal,
    evaluate_iso_recal_on_oof,
    fit_iso_recal,
)
...
OOF_PATH = _ROOT / "data" / "processed" / "v34_oof.parquet"
TEST_PATH = _ROOT / "data" / "processed" / "v34_test.parquet"
OUTPUT_PATH = _ROOT / "submissions" / "archive" / "v34.0_curtailment_iso.csv"
...
def main():
    ...
    print("=" * 72)
    print(f"V34: v32 + availability features + iso-recal  ({len(seeds)} seeds)")
    print("=" * 72)
    ...
    # Mark impossible rows so the availability isotonic ignores them.
    ...
    # Propagate the flag back into combined so add_walk_forward_availability
    # can read it.
    ...
    # ★ V34 addition — walk-forward seasonal availability features
    ...
    avail_cols = availability_columns()
    print(f"    Added {len(avail_cols)} columns: {avail_cols}")

    df_train_full = combined[combined["_split"] == "train"].reset_index(drop=True)
    df_valid_sorted = combined[combined["_split"] == "valid"].reset_index(drop=True)

    # --- Probe → top-K (CF target on Fold-5) -----------------------------
    print("[3/5] Feature selection (probe on Fold-5)...")
    folds = default_folds()
    fold5 = folds[-1]
    ...
    feat_cols_all = [
        ...
        # Drop avail_underprod_mw — it's NaN on valid rows AND it's
        # near-target on train rows (= y - p_expected). Keep only the
        # past-only aggregates, which are leakage-safe.
    ...
    print(f"  Top-{K}: {sum(1 for n in top_k if n.startswith('era5v2_'))} era5v2_*, "
          f"{n_avail_top} avail_* features")
    if n_avail_top:
        print("  avail_* in top-K:")
        imp_lookup = dict(feat_imp)
        for n in top_k:
            if n.startswith("avail_"):
                print(f"    {n}  (gain={imp_lookup[n]:,.0f})")

    # --- CV-bag with OOF + per-fold test preds ---------------------------
    print(f"\n[4/5] Training CV-bag (folds {[i + 1 for i in FOLD_IDS]}, "
          f"seeds={seeds})...")
    test_cf_per_fold = {}
    test_mw_per_fold = {}
    ...
    # --- [5/5] Iso-recal + final blend -----------------------------------
    print("\n[5/5] Iso-recalibration + final blend...")

    # Per-fold leave-one-out evaluation (sanity check the recal helps).
    eval_cf = evaluate_iso_recal_on_oof(oof_wide, leg_pred_col="pred_cf_mw")
    eval_mw = evaluate_iso_recal_on_oof(oof_wide, leg_pred_col="pred_mw_mw")
    print("  Iso-recal OOF (LOO across folds):")
    print("    CF:")
    for _, r in eval_cf.iterrows():
        print(f"      Fold {int(r['fold'])}: {r['before_nmae_pct']:.4f}% → "
              f"{r['after_nmae_pct']:.4f}%  (Δ={r['delta_pp']:+.4f} pp)")
    print(f"      mean Δ: {eval_cf['delta_pp'].mean():+.4f} pp")
    print("    MW:")
    for _, r in eval_mw.iterrows():
        print(f"      Fold {int(r['fold'])}: {r['before_nmae_pct']:.4f}% → "
              f"{r['after_nmae_pct']:.4f}%  (Δ={r['delta_pp']:+.4f} pp)")
    print(f"      mean Δ: {eval_mw['delta_pp'].mean():+.4f} pp")

    # Fit final calibrators on the FULL OOF and apply to averaged test preds.
    ir_cf = fit_iso_recal(oof_wide["pred_cf_mw"].to_numpy(), oof_wide["target_mw"].to_numpy())
    ir_mw = fit_iso_recal(oof_wide["pred_mw_mw"].to_numpy(), oof_wide["target_mw"].to_numpy())
    ...
    pred_cf_mw_raw = np.clip(_from_cf(avg_test_cf, active_valid), 0, CAPACITY_MW)
    pred_mw_mw_raw = np.clip(avg_test_mw, 0, CAPACITY_MW)
    pred_cf_mw_cal = apply_iso_recal(ir_cf, pred_cf_mw_raw)
    pred_mw_mw_cal = apply_iso_recal(ir_mw, pred_mw_mw_raw)

    final_mw = (BLEND_WEIGHT_MW * pred_mw_mw_cal + (1 - BLEND_WEIGHT_MW) * pred_cf_mw_cal)
