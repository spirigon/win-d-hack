"""V85: V75 pipeline + steep-zone features, dual-target CF+MW 50/50 blend.

Key learning: v75's 50/50 CF+MW blend (LB 7.43%) beats CF-only from the same
parquet (LB 7.49%). MW-mode adds 0.06pp via better generalization to test
distribution. Any comparison to v75 must use the same blend.

Changes vs v75:
  + add_all_steep_zone_features() after add_all_advanced_features()
    (Jensen correction, wind CV, cubic spread, GFS direction instability,
     GFS-ECMWF direction diff, steep zone × spread interactions)
  + curtailment weighting (1035 rows, 3.2%)
  - No change to K, seeds, fold structure, or blend ratio

Baseline: V75 = 7.43% LB  (blend OOF Fold-5 = 7.563%)

Outputs:
    data/processed/v85_oof.parquet
    data/processed/v85_test.parquet
    submissions/archive/v85.0_steep_blend.csv

Usage:
    python -m src.training.train_v85_steep_blend
    python -m src.training.train_v85_steep_blend --seeds 5
"""
...
from src.features.steep_zone_features import add_all_steep_zone_features, steep_zone_columns
...
OOF_PATH    = _ROOT / "data" / "processed" / "v85_oof.parquet"
TEST_PATH   = _ROOT / "data" / "processed" / "v85_test.parquet"
OUTPUT_PATH = _ROOT / "submissions" / "archive" / "v85.0_steep_blend.csv"
...
def main() -> None:
    ...
    print("=" * 72)
    print(f"V85: V75 + steep-zone features + curtailment  ({len(seeds)} seeds)")
    print(f"     CF+MW 50/50 blend  (BLEND_WEIGHT_MW={BLEND_WEIGHT_MW})")
    print("=" * 72)
    ...
    combined = add_all_steep_zone_features(combined)     # NEW
    ...
    steep_cols = steep_zone_columns(combined)
    print(f"  Feature groups: nwp={len(nwp_cols)} nasa={len(nasa_cols)} "
          f"mnwp={len(mnwp_cols)} adv={len(adv_cols)} steep={len(steep_cols)}")
    ...
    print(f"\n[2/4] Feature selection probe (Fold-5, K={K})...")
    folds = default_folds()
    fold5 = folds[-1]
    ...
    all_extra_cols = list(dict.fromkeys(nwp_cols + nasa_cols + mnwp_cols + adv_cols + steep_cols))
    feat_cols_all = [
    ...
    print(f"  Feature pool: {len(feat_cols_all)}  ({n_era5v2} era5v2, {len(steep_cols)} steep)")
    ...
    steep_in_topk = [c for c in top_k if c in steep_cols]
    print(f"  Top-{K} selected  (top-3: {top_k[:3]})")
    print(f"  Steep-zone in top-{K}: {len(steep_in_topk)}  {steep_in_topk}")

    # --- CV-bag training (CF + MW) ---
    ...
    for target_mode in ("cf", "mw"):
        ...
        for fold_idx in FOLD_IDS:
            ...
            train_idx2, val_idx2 = split_indices(ft, fold)
            fold_train = ft.iloc[train_idx2]
            fold_val   = ft.iloc[val_idx2]
            ...
            sw     = sample_weight_full[train_idx2]
    ...
    for fid in [3, 4, 5]:
        ...
        n_blend = float(normalized_mae(sub["target_mw"].to_numpy(), sub["pred_blend_mw"].to_numpy()))
        n_cf    = float(normalized_mae(sub["target_mw"].to_numpy(), sub["pred_cf_mw"].to_numpy()))
        print(f"    Fold {fid}: blend={n_blend:.4f}%  CF={n_cf:.4f}%")
