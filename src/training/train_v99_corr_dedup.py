"""V99: V97 + correlation-based near-duplicate removal.

V97 removed 19 byte-identical features (ERA5=ECMWF raw columns).
However, derived features such as era5_ws100_roll_mean_6h and
ecmwf_ws100_roll6h_mean survive byte-identical dedup because they use
different rolling min_periods — yet their correlation is >>0.999.
These near-duplicates still waste K slots.

This version adds a second dedup pass with |r|>0.999 threshold after
byte-identical dedup, using a fast numpy correlation matrix computed on
the fold-5 training data.

K=90, 5 seeds, CPU.  Everything else identical to V97.

Outputs:
    data/processed/v99_oof.parquet
    submissions/archive/v99.0_corr_dedup.csv
"""
...
OOF_PATH    = _ROOT / "data" / "processed" / "v99_oof.parquet"
OUTPUT_PATH = _ROOT / "submissions" / "archive" / "v99.0_corr_dedup.csv"

MARCH_WEIGHT   = 3.0
SEEDS          = SEEDS_5
K              = 90
CORR_THRESHOLD = 0.999  # remove near-duplicates with |r| above this

_GPU_OVERRIDES: dict = {}


def _corr_dedup(feat_cols: list[str], X: np.ndarray, threshold: float) -> list[str]:
    """Greedy correlation dedup: keep first feature in pool order;
    drop any subsequent feature with |r| > threshold vs any kept feature.

    X: (n_rows, n_features) float64 matrix, may contain NaN.
    """
    n, p = X.shape
    # Fill NaN with column mean so matmul is defined
    ...
    X_normed = (X_filled - X_filled.mean(axis=0)) / col_stds  # (n, p), zero-mean unit-std

    # Full correlation matrix: (p, p)
...
def main() -> None:
    ...
    print("=" * 72)
    print(f"V99: corr dedup + GEM/ICON-G features  ({len(SEEDS)} seeds, K={K})")
    print("=" * 72)
    ...
    # Pass 1: byte-identical dedup (catches ERA5=ECMWF raw columns)
    ...
    # Pass 2: correlation dedup (catches near-duplicates like era5/ecmwf rolling features)
