"""V101: CatBoost model trained on V99's feature set.

CatBoost uses symmetric oblivious trees with ordered boosting — fundamentally
different from LightGBM's leaf-wise growth.  Blending V99 (LGBM) + V101
(CatBoost) provides genuine model diversity that cannot be achieved by varying
LGBM hyperparameters.

Feature pipeline: identical to V99 (byte-identical + correlation dedup, GEM/ICON-G).
Training: single CatBoost model per seed per fold (no regime split — CatBoost
          handles non-linearity via tree depth without explicit specialisation).
Blend: 50/50 CF+MW blend, same as V99.

Run AFTER V99 completes (uses same probe to pick K=90 features).

Outputs:
    data/processed/v101_oof.parquet
    submissions/archive/v101.0_catboost.csv
    submissions/archive/v101.1_lgbm_catboost_blend.csv  (50/50 blend with V99)
"""
...
V99_SUB     = _ROOT / "submissions" / "archive" / "v99.0_corr_dedup.csv"
OOF_PATH    = _ROOT / "data" / "processed" / "v101_oof.parquet"
OUTPUT_PATH = _ROOT / "submissions" / "archive" / "v101.0_catboost.csv"
BLEND_PATH  = _ROOT / "submissions" / "archive" / "v101.1_lgbm_catboost_blend.csv"

MARCH_WEIGHT   = 3.0
SEEDS          = SEEDS_5
K              = 90
CORR_THRESHOLD = 0.999
BLEND_WEIGHT_MW = 0.0  # CF-only proven optimal by blend_weight_optimizer (v97, v99)

# CatBoost hyperparameters (regression, CPU)
CB_PARAMS = {
    "loss_function": "RMSE",
    "iterations": 4000,
    "learning_rate": 0.05,
...
def _train_cb_fold(X_tr, y_tr, X_va, y_va, X_test, feat_cols, sw, seeds):
    """Train CatBoost with multiple seeds, return averaged val+test predictions."""
    vp, tp = [], []
    for s in seeds:
        cb = CatBoostRegressor(**{**CB_PARAMS, "random_seed": s})
        pool_tr = Pool(data=X_tr, label=y_tr, weight=sw, feature_names=feat_cols)
        pool_va = Pool(data=X_va, label=y_va, feature_names=feat_cols)
        cb.fit(pool_tr, eval_set=pool_va, early_stopping_rounds=CB_EARLY_STOP)
        vp.append(cb.predict(X_va))
        tp.append(cb.predict(X_test))
    return np.mean(vp, axis=0), np.mean(tp, axis=0)


def _train_lgbm_fold(X_tr, y_tr, X_va, y_va, X_test, feat_cols, ws_tr, sw, seeds):
    """Regime-split LGBM ensemble (same as V99)."""
...
def main() -> None:
    ...
    print("=" * 72)
    print(f"V101: CatBoost + LGBM blend  ({len(SEEDS)} seeds, K={K})")
    print("=" * 72)
    ...
    print(f"\n[3/4] Training LGBM + CatBoost CV-bag...")
    test_cf_lgbm: dict[int, np.ndarray] = {}
    test_mw_lgbm: dict[int, np.ndarray] = {}
    test_cf_cb:   dict[int, np.ndarray] = {}
    test_mw_cb:   dict[int, np.ndarray] = {}
    ...
    for target_mode in ("cf", "mw"):
        ...
        for fold_idx in FOLD_IDS:
            ...
            # LGBM (regime split)
            lgbm_val, lgbm_test = _train_lgbm_fold(
                X_tr, y_tr, X_va, y_va, X_test, top_k, ws_tr, sw, SEEDS,
            )
            # CatBoost (single model per seed, no regime split)
            cb_val, cb_test = _train_cb_fold(
                X_tr.astype(np.float64), y_tr.astype(np.float64),
                X_va.astype(np.float64), y_va.astype(np.float64),
                X_test.astype(np.float64), top_k, sw.astype(np.float64), SEEDS,
            )

            # Blend LGBM and CatBoost 50/50
            val_pred  = 0.5 * lgbm_val  + 0.5 * cb_val
            test_pred = 0.5 * lgbm_test + 0.5 * cb_test

            if target_mode == "cf":
                val_mw = np.clip(_from_cf(val_pred, active_va), 0, CAPACITY_MW)
                lgbm_val_mw = np.clip(_from_cf(lgbm_val, active_va), 0, CAPACITY_MW)
                cb_val_mw   = np.clip(_from_cf(cb_val, active_va), 0, CAPACITY_MW)
                test_cf_lgbm[fold_idx] = lgbm_test
                test_cf_cb[fold_idx]   = cb_test
            else:
                val_mw = np.clip(val_pred, 0, CAPACITY_MW)
                lgbm_val_mw = np.clip(lgbm_val, 0, CAPACITY_MW)
                cb_val_mw   = np.clip(cb_val, 0, CAPACITY_MW)
                test_mw_lgbm[fold_idx] = lgbm_test
                test_mw_cb[fold_idx]   = cb_test

            fold_nmae     = float(normalized_mae(y_va_mw, val_mw))
            lgbm_fold_nmae = float(normalized_mae(y_va_mw, lgbm_val_mw))
            cb_fold_nmae   = float(normalized_mae(y_va_mw, cb_val_mw))
            print(f"    Fold {fold_idx + 1}: {target_mode.upper()} "
                  f"blend={fold_nmae:.4f}%  LGBM={lgbm_fold_nmae:.4f}%  "
                  f"CB={cb_fold_nmae:.4f}%  ({time.time() - t0:.0f}s)")
    ...
    def _make_blend(cf_per_fold, mw_per_fold, active):
        avg_cf = np.mean([cf_per_fold[i] for i in FOLD_IDS], axis=0)
        avg_mw = np.mean([mw_per_fold[i] for i in FOLD_IDS], axis=0)
        pred_cf_mw = np.clip(_from_cf(avg_cf, active), 0, CAPACITY_MW)
    ...
    # LGBM+CatBoost blended predictions
    lgbm_blend = _make_blend(test_cf_lgbm, test_mw_lgbm, active_valid)
    cb_blend   = _make_blend(test_cf_cb,   test_mw_cb,   active_valid)
    final_mw   = (0.5 * lgbm_blend + 0.5 * cb_blend)
    ...
    print(f"  CatBoost submission: {OUTPUT_PATH}")

    # Blend with V99 if available
    if V99_SUB.exists():
        v99_preds = pd.read_csv(V99_SUB)
        pred_col  = [c for c in v99_preds.columns if c != v99_preds.columns[0]][0]
        v99_arr   = v99_preds[pred_col].to_numpy(dtype=np.float64)
        super_blend = np.clip(0.5 * po + 0.5 * v99_arr, 0.0, CAPACITY_MW)
        write_submission(super_blend, BLEND_PATH, expected_rows=len(df_valid))
        print(f"  LGBM+CatBoost super-blend: {BLEND_PATH}")
    else:
        print(f"  V99 submission not found at {V99_SUB} — skipping super-blend")
