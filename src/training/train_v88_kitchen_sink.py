"""V88: V75 + air density + smeared power + seasonal interactions + CF/MW blend.

Combines all new physics-motivated features in one run:
  - air_density_norm, v120_density, v_eff_density, wpd_density        (6 cols)
  - smear_p{1,2,3,4}_{per_turb_kw,farm_mw,excess_mw}                (12 cols)
  - month_sin/cos_x_veff, is_winter_x_veff/ws120, month_x_ws120,
    season_x_wpd                                                       (6 cols)

K=80 feature probe selects the best 80 from the full pool, so noise features
are automatically filtered out.  CF+MW 50/50 blend for fair v75 comparison.

Outputs:
    data/processed/v88_oof.parquet
    submissions/archive/v88.0_kitchen_sink.csv
"""
...
OOF_PATH    = _ROOT / "data" / "processed" / "v88_oof.parquet"
OUTPUT_PATH = _ROOT / "submissions" / "archive" / "v88.0_kitchen_sink.csv"
...
def main() -> None:
    ...
    print("=" * 72)
    print(f"V88: V75 + air density + smeared power + seasonal  ({len(SEEDS)} seeds, K={K})")
