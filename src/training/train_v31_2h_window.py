"""V31: v29 + 2h ERA5 rolling window.

Adds era5_ws100_roll_mean_2h and era5_ws100_roll_std_2h to the feature set.
The 2h mean ranked #9 in the v30 ablation and improved Fold-5 by +0.013 pp
(7.5903% → 7.5772%).

Fold-5 target: ~7.577%

Usage:
    python -m src.training.train_v31_2h_window
"""
...
SUBMISSION_PATH = _ROOT / "submissions" / "archive" / "v31.0_2h_window.csv"
...
def _add_era5_rolling(df):
    """ERA5 rolling features: base 3/6/12/24h + new 2h window."""
    df = df.copy()
    ws = df["era5_wind_speed_100m"]
    pres = df["era5_pressure_msl"]
    for w in [2, 3, 6, 12, 24]:  # 2h added
...
def main() -> None:
    set_global_seed(42)
    print("V31: v29 + 2h ERA5 rolling window")
    print("=" * 60)
    ...
    rank_2h = top_k.index("era5_ws100_roll_mean_2h") + 1 if "era5_ws100_roll_mean_2h" in top_k else "not in top-K"
    print(f"  Top-{K} selected. era5_ws100_roll_mean_2h rank: {rank_2h}")
