"""V87: V75 features + air density + smeared theoretical power + CF/MW 50/50 blend.

New vs V86 (air density only):
  - smear_p1_* : (P(v-1) + P(v+1)) / 2  — farm MW, per-turbine kW, excess MW
  - smear_p2_* : (P(v-2) + P(v+2)) / 2
  - smear_p3_* : (P(v-3) + P(v+3)) / 2
  - smear_p4_* : (P(v-4) + P(v+4)) / 2

Smeared power corrects for NWP forecast uncertainty: because the power curve is
convex at 7-12 m/s, E[P(v±δ)] > P(v), so the model gains a trainable proxy for
the Jensen correction it needs in the steep zone.

Everything else identical to V75:
  - CF + MW dual-target, BLEND_WEIGHT_MW=0.5
  - Walk-forward availability, curtailment, March × 3.0
  - Feature selection probe K=80
  - Folds [3, 4, 5], 3 seeds

Outputs:
    data/processed/v87_oof.parquet
    submissions/archive/v87.0_smeared_power.csv
"""
...
from src.features.air_density_features import add_air_density_features, air_density_columns
...
from src.features.smeared_power_features import (
    add_smeared_power_features,
    smeared_power_columns,
    DELTA_V_LIST,
)
...
from src.training.train_v32_era5v2 import (
    LGBM_PARAMS, K, BLEND_WEIGHT_MW, FOLD_IDS, SEEDS_3,
...
OOF_PATH    = _ROOT / "data" / "processed" / "v87_oof.parquet"
OUTPUT_PATH = _ROOT / "submissions" / "archive" / "v87.0_smeared_power.csv"
...
def main() -> None:
    ...
    print("=" * 72)
    print(f"V87: V75 + air density + smeared power  ({len(SEEDS)} seeds, K={K})")
    print(f"     Delta-V values: {DELTA_V_LIST} m/s")
    print("=" * 72)
    ...
    # --- New features: air density + smeared power ---
    combined = add_air_density_features(combined)
    combined = add_smeared_power_features(combined)
    ...
    dens_cols  = air_density_columns(combined)
    smear_cols = smeared_power_columns(combined)

    print(f"  Feature groups: nwp={len(nwp_cols)} nasa={len(nasa_cols)} "
          f"mnwp={len(mnwp_cols)} adv={len(adv_cols)} "
          f"density={len(dens_cols)} smear={len(smear_cols)}")
    ...
    all_extra_cols = list(dict.fromkeys(
        nwp_cols + nasa_cols + mnwp_cols + adv_cols + dens_cols + smear_cols
    ))
    feat_cols_all = [
    ...
    in_topk_smear = [c for c in top_k if c in smear_cols]
    in_topk_dens  = [c for c in top_k if c in dens_cols]
    print(f"  Smear features in top-{K}: {len(in_topk_smear)}  {in_topk_smear}")
    print(f"  Density features in top-{K}: {len(in_topk_dens)}  {in_topk_dens}")

    # --- CV-bag training (CF + MW dual-target) ---
