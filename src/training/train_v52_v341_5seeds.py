"""V52: V34.1 (LB 7.56 champion) at 5 seeds instead of 3.

V34.1 is our best LB submission so far. Going from 3 seeds to 5 seeds
typically buys 0.005-0.015 pp on LB by reducing seed variance. This is
the lowest-risk move available: same architecture, same features, same
hyperparameters — just more averaging.

Outputs:
    data/processed/v52_oof.parquet
    data/processed/v52_test.parquet
    submissions/archive/v52.0_v341_5seeds.csv
"""
...
OOF_PATH = _ROOT / "data" / "processed" / "v52_oof.parquet"
TEST_PATH = _ROOT / "data" / "processed" / "v52_test.parquet"
OUTPUT_PATH = _ROOT / "submissions" / "archive" / "v52.0_v341_5seeds.csv"
...
def main():
    ...
    print("=" * 72)
    print(f"V52: V34.1 architecture, 5 seeds  (LB-7.56 champion + variance reduction)")
    print("=" * 72)

    print("\n[1/4] Loading + building features (v34.1 path)...")
    ...
    # --- CV-bag (5 seeds) ----------------------------------------------
    print(f"\n[3/4] Training CV-bag (folds {[i + 1 for i in FOLD_IDS]}, seeds={SEEDS_5})...")
    test_cf_per_fold = {}
    test_mw_per_fold = {}
    ...
    for target_mode in ("cf", "mw"):
        ...
        for fold_idx in FOLD_IDS:
            ...
            val_pred, test_pred = _train_fold_ensemble(
                X_tr, y_tr, X_va, y_va, X_test, top_k, ws_tr, sw, SEEDS_5,
            )
            if target_mode == "cf":
