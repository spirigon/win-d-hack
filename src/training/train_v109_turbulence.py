"""V109: V97b + turbulence / NWP-ensemble-rank / density-WPD features.

Adds add_all_turbulence_features() to the feature pool before the K=90 probe.
New feature groups:
  - Multi-model turbulence intensity (gust/wind from GFS, ECMWF, ERA5, ICON)
  - NWP ensemble rank (where hackathon NWP sits among all models)
  - Density-corrected WPD from GFS and ECMWF using their own T+P+ws
  - Regime features (steep zone, rated zone, night interactions)

CF-only blend (BLEND_WEIGHT_MW=0.0), byte-dedup only (no corr dedup), K=90, 5 seeds.

Outputs:
    data/processed/v109_oof.parquet
    submissions/archive/v109.0_turbulence.csv
"""
...
OOF_PATH    = _ROOT / "data" / "processed" / "v109_oof.parquet"
OUTPUT_PATH       = _ROOT / "submissions" / "archive" / "v109.0_turbulence.csv"
OUTPUT_BLEND_PATH = _ROOT / "submissions" / "archive" / "v109.1_blend50.csv"
...
def main() -> None:
    ...
    print("=" * 72)
    print(f"V109: dedup + GEM/ICON-G + turbulence features  ({len(SEEDS)} seeds, K={K})")
    print("=" * 72)
    ...
    combined = add_all_turbulence_features(combined)  # gust-TI, NWP rank, density-WPD, regimes
    ...
    all_extra_cols = list(dict.fromkeys(
        nwp_cols + nasa_cols + mnwp_cols + adv_cols + hub_cols + seas_cols + gem_cols + turb_cols
    ))
    feat_cols_all = [
    ...
    for label, cols in [
        ...
        ("gem_icon", gem_cols), ("turbulence", turb_cols),
    ]:
    ...
    # 50/50 blend — LB favors blend over CF-only by ~0.04pp (confirmed v97b LB experiment)
