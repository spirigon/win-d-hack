"""V113: K=120 with byte-dedup only — more features, cleaner dedup than v105.

V105 used K=120 but with corr-dedup which costs ~0.036pp on F5.
This version uses byte-identical dedup only (same as v97b) with K=120.
Hypothesis: ranking 91-120 features may include real signal that K=90 misses.

All other settings identical to V97b:
  - Byte-identical dedup (no corr dedup)
  - GEM/ICON-G inter-model features
  - CF-only blend (BLEND_WEIGHT_MW=0.0)
  - 5 seeds, regime-split training

Outputs:
    data/processed/v113_oof.parquet
    submissions/archive/v113.0_k120_bytededup.csv
"""
...
OOF_PATH    = _ROOT / "data" / "processed" / "v113_oof.parquet"
OUTPUT_PATH = _ROOT / "submissions" / "archive" / "v113.0_k120_bytededup.csv"

MARCH_WEIGHT = 3.0
BLEND_WEIGHT_MW = 0.0  # CF-only: optimal per blend_weight_optimizer
SEEDS = SEEDS_5
K = 120  # wider probe, byte-dedup only (no corr dedup penalty)
...
def main() -> None:
    ...
    print("=" * 72)
    print(f"V113: K=120 + byte-dedup only  ({len(SEEDS)} seeds, K={K})")
    print("=" * 72)
    ...
    # Byte-identical dedup only (no corr dedup — corr dedup costs ~0.036pp on F5)
