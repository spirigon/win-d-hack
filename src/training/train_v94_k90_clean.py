"""V94: V92 features (hub 85m + seasonal) at K=90, clean pool (no smear/density).

Hypothesis: v92 (K=80) beat v90 (K=90+smear/density) despite fewer features.
Pool cleanliness (no 0/K noise features) at wider K should give best of both.
Same feature set as v92 — hub+seasonal — but K=90 to capture marginal base features.

Outputs:
    data/processed/v94_oof.parquet
    submissions/archive/v94.0_k90_clean.csv
"""
...
OOF_PATH    = _ROOT / "data" / "processed" / "v94_oof.parquet"
OUTPUT_PATH = _ROOT / "submissions" / "archive" / "v94.0_k90_clean.csv"

MARCH_WEIGHT = 3.0
SEEDS = SEEDS_5
K = 90  # override: wider K, same clean pool as v92
...
def main() -> None:
    ...
    print("=" * 72)
    print(f"V94: hub+seasonal, K=90, clean pool  ({len(SEEDS)} seeds, K={K})")
    print("=" * 72)
