"""V83: V75 pipeline + steep-zone targeted features (K=80, March x3, CF-only).

The 7-10 m/s regime is the key bottleneck (nMAE=24.89%, r=0.68).
New features:
  - Jensen correction  : 3*(ens3_std/ws)^2  — E[P(v)] > P(E[v]) correction
  - Wind CV            : ens3_std/ens3_mean  — relative NWP uncertainty
  - Cubic energy spread: |gfs_cube - ecmwf_cube|  — energy-domain disagreement
  - GFS dir instability: circular variance at 12h and 24h windows
...
Baseline: V75 = 7.43% LB  (Fold5 OOF = 7.525%)

Outputs:
    data/processed/v83_oof.parquet
    data/processed/v83_test.parquet
    submissions/archive/v83.0_steep_zone.csv

Usage:
    python -m src.training.train_v83_steep_zone
    python -m src.training.train_v83_steep_zone --seeds 5
"""
...
OOF_PATH    = _ROOT / "data" / "processed" / "v83_oof.parquet"
TEST_PATH   = _ROOT / "data" / "processed" / "v83_test.parquet"
OUTPUT_PATH = _ROOT / "submissions" / "archive" / "v83.0_steep_zone.csv"
...
def main() -> None:
    ...
    print("=" * 72)
    print(f"V83: V75 features + steep-zone features  ({len(seeds)} seeds, K={K})")
    print("=" * 72)
    ...
    all_extra = list(dict.fromkeys(nwp_cols + nasa_cols + mnwp_cols + adv_cols + steep_cols))
    feat_cols_all = [
    ...
    y_v_cf5 = _to_cf(df_v_[TARGET_COL].to_numpy(dtype=np.float32), a_va5)
    ...
    dt_ = lgb.Dataset(df_t_[feat_cols_all].to_numpy(dtype=np.float32),
                      label=y_tr5, weight=sw5, feature_name=feat_cols_all, free_raw_data=False)
    dv_ = lgb.Dataset(df_v_[feat_cols_all].to_numpy(dtype=np.float32),
                      label=y_v_cf5, feature_name=feat_cols_all, free_raw_data=False)
    probe = lgb.train(
    ...
    print(f"  Steep-zone features in top-{K}: {len(steep_in_topk)}  {steep_in_topk}")
