"""V63: V34.1 with the intra-fold availability leak fixed.

The only change vs ``train_v34_curtailment``: ``add_walk_forward_availability``
receives ``freeze_after_ts=fold.train_end`` so validation rows of each fold
cannot contribute underprod observations to the (month, hour) cumulative
table that later validation rows of the same fold read.

The OOF Fold-5 nMAE is therefore a truer estimate of LB. v34.1 was measuring
~7.617% (OOF) → LB 7.56. Expected v63 OOF ≈ 7.63%, LB may be similar.

This is the honest v34.1 and should be used as the new baseline for any
further experiments.

Outputs:
    data/processed/v63_oof.parquet
    data/processed/v63_test.parquet
    submissions/archive/v63.0_leak_fix.csv

Usage:
    python -m src.training.train_v63_leak_fix --seeds 5
"""
...
OOF_PATH  = _ROOT / "data" / "processed" / "v63_oof.parquet"
TEST_PATH = _ROOT / "data" / "processed" / "v63_test.parquet"
OUTPUT_PATH = _ROOT / "submissions" / "archive" / "v63.0_leak_fix.csv"
...
def main():
    ...
    print("=" * 72)
    print(f"V63: V34.1 with availability leak fix  ({len(seeds)} seeds)")
    print("=" * 72)

    print("\n[1/4] Loading + building base features (no availability yet)...")
    ...
    # Feature selection probe uses Fold-5 with the leak fix applied.
    print("[2/4] Feature selection (probe on Fold-5, leak-fixed availability)...")
    folds = default_folds()
    fold5 = folds[-1]

    # Compute availability frozen at fold5.train_end for the probe.
    ...
    # CV-bag: each fold recomputes availability frozen at that fold's train_end.
    ...
    for target_mode in ("cf", "mw"):
        ...
        for fold_idx in FOLD_IDS:
            ...
            # Recompute availability with leak fix for this fold.
            ...
            val_pred, test_pred = _train_fold_ensemble(X_tr, y_tr, X_va, y_va,
    ...
    # Test parquet + submission.
    # Use availability frozen at train_end (full training data) for the test prediction.
