"""V75: V68 (best 7.51) + GFS + ECMWF IFS + full multi-NWP consensus + advanced interactions.

New vs V68 (March 3x + ICON-EU):

    1. GFS Global Forecast System  (100% clean, 2022-2026 Q1)
       → gfs_ws100m, gfs_vs_era5_100, gfs_ratio_era5_100, gfs_vs_nwp_100
       → gfs_ws100m_trend_6h/12h, gfs_dir100m_sin/cos

    2. ECMWF IFS  (100% clean, 2022-2026 Q1)
       → ecmwf_ws100m, ecmwf_vs_era5_100, ecmwf_vs_nwp_100
       → ecmwf_ws100m_trend_6h/12h, gfs_vs_ecmwf_100, ecmwf_gfs_mean

    3. 3/4-model consensus (ERA5 + GFS + ECMWF [+ ICON-EU])
       → ens3_ws100_{mean,std,range}, hackathon_vs_ens3, hackathon_ratio_ens3
       → ens3_spread_flag, ens4_ws100_{mean,std}, icon_shift_ens3

    4. ERA5v2 hub-height interactions
       → era5v2_ws120_vs_nwp120 (pressure-level ERA5 at exact 120m vs NWP)
       → era5v2_llj_x_ws120, era5v2_blh_x_ws120, era5v2_cape_x_ws120
       → era5v2_850hpa_x_ws120, era5v2_925_850_shear

    5. Multi-model direction agreement
       → gfs_era5_dir_agree, ecmwf_era5_dir_agree, multi_dir_consensus

    6. Atmospheric stability interactions
       → is_inversion, stability_x_ws120, ri_x_ws120, hellmann_x_ws120

    7. Extended rolling (GFS/ECMWF/spread) + direction change features

Outputs:
    data/processed/v75_oof.parquet
    data/processed/v75_test.parquet
    submissions/archive/v75.0_full_ensemble.csv

Usage:
    python -m src.training.train_v75_full_ensemble --seeds 3
    python -m src.training.train_v75_full_ensemble --seeds 5
"""
...
from src.features.advanced_interactions import (
    add_all_advanced_features,
    advanced_interaction_columns,
)
...
OOF_PATH    = _ROOT / "data" / "processed" / "v75_oof.parquet"
TEST_PATH   = _ROOT / "data" / "processed" / "v75_test.parquet"
OUTPUT_PATH = _ROOT / "submissions" / "archive" / "v75.0_full_ensemble.csv"
...
def main() -> None:
    ...
    print("=" * 72)
    print(f"V75: V68 + GFS + ECMWF + consensus + advanced interactions  ({len(seeds)} seeds)")
    print(f"     March weighting: x{MARCH_WEIGHT}")
    print("=" * 72)
    ...
    combined = merge_era5_v2(combined)        # era5v2_* (pressure levels, BLH, CAPE, LLJ, etc.)
    combined = merge_nwp_ensemble(combined)   # ICON-EU (icon_*), Open-Meteo ensemble (om_*)
    combined = merge_nasa_merra2(combined)    # NASA MERRA-2 (nasa_*)
    combined = merge_gfs(combined)            # GFS 100% clean (gfs_*)
    combined = merge_ecmwf_ifs(combined)      # ECMWF IFS 100% clean (ecmwf_*)
    combined = build_nwp_consensus(combined)  # Multi-model consensus (ens3_*, ens4_*, hackathon_vs_ens3)
    ...
    print(f"  ICON-EU / OM ensemble features:  {len(nwp_cols)}")
    print(f"  NASA MERRA-2 features:            {len(nasa_cols)}")
    print(f"  GFS + ECMWF + consensus features: {len(mnwp_cols)}")
    print(f"  Advanced interaction features:    {len(adv_cols)}")
    ...
    for col, label in [
        ...
        ("gfs_ws100m",         "GFS"),
        ("ecmwf_ws100m",       "ECMWF"),
        ("hackathon_vs_ens3",  "hackathon_vs_ens3"),
    ]:
        if col in combined.columns:
            cov = combined[col].notna().mean() if combined[col].dtype != object else 0
            print(f"  {label} coverage: {cov:.1%}")

    # --- Impossible rows & sample weights ---
    ...
    print("[2/4] Feature selection probe (Fold-5)...")
    folds = default_folds()
    fold5 = folds[-1]
    ...
    all_extra_cols = list(dict.fromkeys(nwp_cols + nasa_cols + mnwp_cols + adv_cols))
    feat_cols_all = [
    ...
    print(f"  Feature pool: {len(feat_cols_all)}  "
          f"({n_era5v2} era5v2_*, {len(nwp_cols)} nwp_*, "
          f"{len(nasa_cols)} nasa_*, {len(mnwp_cols)} multi_nwp_*, "
          f"{len(adv_cols)} adv_*)")
    ...
    # Report which new features made it into top-K
    for group, cols in [("NASA", nasa_cols), ("GFS/ECMWF/consensus", mnwp_cols), ("advanced", adv_cols)]:
        in_topk = [c for c in top_k if c in cols]
        print(f"  {group} in top-{K}: {in_topk}")
