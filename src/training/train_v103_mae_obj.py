"""V103: V99 pipeline + L1/MAE objective instead of RMSE.

nMAE evaluation uses absolute error — training with MAE loss (regression_l1)
aligns the objective with the metric.  Every other design choice is identical
to V99 (corr dedup, GEM/ICON-G, K=90, 5 seeds, CF+MW blend).

The main LGBM param change: objective='regression_l1' (predicts medians).
We also drop learning_rate slightly and increase early-stopping patience
because L1 loss has flatter gradients near the optimum.

Outputs:
    data/processed/v103_oof.parquet
    submissions/archive/v103.0_mae_obj.csv
"""
...
OOF_PATH    = _ROOT / "data" / "processed" / "v103_oof.parquet"
OUTPUT_PATH = _ROOT / "submissions" / "archive" / "v103.0_mae_obj.csv"
...
# MAE-specific LGBM overrides — same as LGBM_PARAMS but with MAE objective
# and slightly longer patience (flat gradient near optimum)
_MAE_OVERRIDES = {
...
def _train_fold_ensemble(X_tr, y_tr, X_va, y_va, X_test, feat_cols, ws_tr, sw, seeds):
    """Regime-split ensemble with MAE objective."""
    regime_val, regime_test = {}, {}
    base_params = {**LGBM_PARAMS.__dict__, **_MAE_OVERRIDES}
    for name, (lo, hi) in [("low_0_7", (0, 7)), ("mid_4_12", (4, 12)), ("high_8_25", (8, 25))]:
        ...
        vp, tp = [], []
        for s in seeds:
            cfg = LGBMConfig(**{**base_params, "seed": s})
...
def main() -> None:
    ...
    print("=" * 72)
    print(f"V103: MAE objective + corr dedup + GEM/ICON-G  ({len(SEEDS)} seeds, K={K})")
    print("=" * 72)
    ...
    # Probe with MAE objective too (same probe model = better probe alignment)
    probe_params = {**LGBM_PARAMS.__dict__, **_MAE_OVERRIDES, "seed": 42}
    cfg0 = LGBMConfig(**probe_params)
    ...
    print(f"\n[3/4] Training CV-bag (folds {[i + 1 for i in FOLD_IDS]}, "
          f"CF+MW blend, seeds={SEEDS}, objective=MAE)...")
