"""V66: V63 + winter month sample weighting (Jan×2, Feb×2, Mar×3).

Test set is Jan-Mar 2026; Fold 5 is Jan-Mar 2025 — exact same months.
Weighting all three winter months forces the regime specialists to resolve
winter wind-power dynamics more precisely, directly targeting the test
distribution.

Per-month Fold-5 nMAE (v32):
    Jan  6.95%  →  upweight ×2   (2x = Jan in-regime: 4.0)
    Feb  7.33%  →  upweight ×2
    Mar  8.42%  →  upweight ×3   (March has largest curtailment tail)

Outputs:
    data/processed/v66_oof.parquet
    data/processed/v66_test.parquet
    submissions/archive/v66.0_winter_weighted.csv

Usage:
    python -m src.training.train_v66_winter_weighted --seeds 5
    python -m src.training.train_v66_winter_weighted --seeds 3   # fast smoke-test
"""
...
OOF_PATH    = _ROOT / "data" / "processed" / "v66_oof.parquet"
TEST_PATH   = _ROOT / "data" / "processed" / "v66_test.parquet"
OUTPUT_PATH = _ROOT / "submissions" / "archive" / "v66.0_winter_weighted.csv"
...
def main() -> None:
    ...
    print("=" * 72)
    print(f"V66: V63 + winter weighting (Jan/Feb x{JAN_FEB_WEIGHT}, Mar x{MARCH_WEIGHT})  ({len(seeds)} seeds)")
    print("=" * 72)
    ...
    # Winter weighting: Jan + Feb × 2, March × 3.
    ...
    print(f"  Jan+Feb rows upweighted {JAN_FEB_WEIGHT}x: {n_janfeb} ({100*n_janfeb/total:.1f}%)")
    print(f"  March rows upweighted {MARCH_WEIGHT}x: {n_march} ({100*n_march/total:.1f}%)")
