"""V97b: V94 + duplicate-column removal + GEM/ICON-Global inter-model features.

Two targeted improvements:
  1. ERA5 and ECMWF IFS are byte-identical in this dataset → our "4-model"
     consensus is really 3 distinct sources.  The probe wastes top-K slots on
     duplicate columns.  Deduplication before probe gives cleaner K coverage.
  2. GEM Global (Canada) and ICON-Global provide genuine independent forecasts
     at 10m (~78-79% coverage from Nov 2022).  Features: extrapolated ws100m,
     inter-model disagreement vs ERA5, multi-source spread as uncertainty signal.

K=90, 5 seeds, CPU.

Outputs:
    data/processed/v97b_oof.parquet
    submissions/archive/v97b.0_cfonly.csv
"""
...
OOF_PATH         = _ROOT / "data" / "processed" / "v97b_oof.parquet"
OUTPUT_PATH      = _ROOT / "submissions" / "archive" / "v97b.0_cfonly.csv"
OUTPUT_BLEND_PATH = _ROOT / "submissions" / "archive" / "v97b.1_blend50.csv"

MARCH_WEIGHT = 3.0
BLEND_WEIGHT_MW = 0.0  # CF-only: optimal per blend_weight_optimizer (OOF-based)
SEEDS = SEEDS_5
K = 90  # wider K, dedup + GEM/ICON-G features
...
def main() -> None:
    ...
    print("=" * 72)
    print(f"V97b: dedup + GEM/ICON-G features  ({len(SEEDS)} seeds, K={K})")
    print("=" * 72)
    ...
    combined = add_gem_icon_features(combined)     # GEM/ICON-G inter-model disagreement
    ...
    # Deduplicate byte-identical columns (ERA5 = ECMWF in this dataset → wastes K slots)
    ...
    # Also write 50/50 blend submission for LB comparison (historical finding: blend > CF-only on LB)
    blend50_mw = np.clip(
        0.5 * pred_mw_mw + 0.5 * pred_cf_mw, 0.0, CAPACITY_MW,
    ).astype(np.float64)
    ...
    print(f"  Blend-50/50 submission: {OUTPUT_BLEND_PATH}")
    print(f"  Blend-50/50 mean: {blend50_mw.mean():.2f} MW")

    # OOF comparison: CF-only vs 50/50 blend on Fold 5
    ...
    print(f"  Submit v97b.0_cfonly.csv if CF-only is better, v97b.1_blend50.csv if blend is better")
