"""V76: V75 + power lags + turbulence features + curtailment mask + CF-only.

Changes from V75 (Fold-5 CF-only 7.525%, 3 seeds):

    1. OOF-chain power lag features  [biggest expected gain]
       lag_power_{1,3,6,12,24}h — actual for train, v75 predictions for valid
       power_roll{6,12,24}h_mean/std, power_ramp_{1,3}h
       Target lag-1 autocorrelation = 0.947 — strongest signal not yet used.

    2. Curtailment-aware sample weights  [replaces ad-hoc outlier zeroing]
       CurtailmentMask flags persistent underproduction events (ws>5m/s but
       power <40% of isotonic baseline for 3+ consecutive hours).
       Flagged rows are zeroed (same as _is_impossible) so the power curve
       fit is not corrupted.

    3. Multi-model turbulence + ensemble rank + density-corrected WPD
       gfs/ecmwf/era5/icon TI proxies, cross-model TI spread,
       NWP rank in 5-model ensemble (high rank → likely over-estimated),
       GFS+ECMWF WPD computed from their own T+P+ws,
       Wind regime soft indicators (steep-zone, rated, night-hour density)

    4. K = 120 (was 80) — richer feature set needs more top features

    5. CF-only blend (MW blend weight = 0.0)
       OOF grid search confirmed w=0.0 is optimal (7.525% vs 7.563% at 0.5).

Outputs:
    data/processed/v76_oof.parquet
    data/processed/v76_test.parquet
    submissions/archive/v76.0_power_lags.csv

Usage:
    python -m src.training.train_v76_power_lags --seeds 3
    python -m src.training.train_v76_power_lags --seeds 5
"""
...
V75_TEST    = _ROOT / "data" / "processed" / "v75_test.parquet"
OOF_PATH    = _ROOT / "data" / "processed" / "v76_oof.parquet"
TEST_PATH   = _ROOT / "data" / "processed" / "v76_test.parquet"
OUTPUT_PATH = _ROOT / "submissions" / "archive" / "v76.0_power_lags.csv"

MARCH_WEIGHT = 3.0
K = 120                # expanded feature budget (was 80)
BLEND_WEIGHT_MW = 0.0  # CF-only — OOF grid search confirmed optimal
...
def main() -> None:
    ...
    print("=" * 72)
    print(f"V76: V75 + power lags + turbulence + curtailment + K={K} + CF-only  ({len(seeds)} seeds)")
    print("=" * 72)
    ...
    # --- Power lag features (uses v75 test predictions for valid rows) ---
    v75_test_exists = V75_TEST.exists()
    combined = build_power_lag_features(
        combined,
        test_parquet=V75_TEST if v75_test_exists else None,
    )
    pl_cols = power_lag_columns(combined)
    if not v75_test_exists:
        print("  WARNING: v75_test.parquet not found — valid lag features will be 0")
    else:
        print(f"  Power lag features: {len(pl_cols)}  (v75 test predictions loaded)")
    ...
    # --- Impossible rows (legacy) + curtailment mask ---
    ...
    # Report new feature group contributions
    for group, cols in [
    ...
    # --- CV-bag training (CF-only target) ---
