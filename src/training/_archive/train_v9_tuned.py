"""V9: v8 wake features + v8-tuned params + 10-seed ensemble.

Best single-seed Fold-5 with tuned params: 7.94%
Expected ensemble: ~7.91%

Usage:
    python -m src.training.train_v9_tuned
"""
...
SUBMISSION_PATH = _ROOT / "submissions" / "archive" / "v9.0_wake_tuned.csv"
...
def main():
    ...
    # v8-tuned params.
    config = LGBMConfig(
        num_leaves=374, min_data_in_leaf=133, learning_rate=0.02152,
        feature_fraction=0.807, bagging_fraction=0.560, bagging_freq=4,
        lambda_l1=0.00617, lambda_l2=0.000359,
    ...
    best_iters = []
    for seed in SEEDS:
        b = _train_single(X_tr, y_tr, X_va, y_va, top_k, seed, config)
    ...
    print(f"\n  >>> Fold-5 10-seed ensemble: {normalized_mae(y_va, ens):.4f} % <<<")
    ...
    boosters = [_train_full(X_full, y_full, top_k, s, n_rounds, config) for s in SEEDS]
    ...
    blend2 = np.clip((po + v80) / 2.0, 0, CAPACITY_MW)
    write_submission(blend2, _ROOT / "submissions" / "archive" / "v9.1_v8_blend.csv", expected_rows=len(df_valid))
    blend3 = np.clip((po + v80 + v70) / 3.0, 0, CAPACITY_MW)
    write_submission(blend3, _ROOT / "submissions" / "archive" / "v9.2_triple_blend.csv", expected_rows=len(df_valid))

    MODEL_DIR.mkdir(parents=True, exist_ok=True)
    for i, b in enumerate(boosters):
        b.save_model(str(MODEL_DIR / f"lgbm_v9_seed{SEEDS[i]}.txt"))
    print("\nDone.")
