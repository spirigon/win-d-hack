"""V8: Outlier-cleaned training + curtailment features + physics + multi-seed.

Improvements over v7:
1. Remove ~165 physically impossible training rows (all wind <4, power >20).
2. Down-weight 2022 rows (lower data quality).
3. Add curtailment risk features (weather-only, inference-safe).
4. Retrain with v7-tuned hyperparameters + 5-seed ensemble.
5. Compute power curve ONLY on clean rows so the model learns a sharper curve.

Usage:
    python -m src.training.train_v8
"""
...
from src.data.outliers import compute_training_weights, identify_impossible_rows
...
SUBMISSION_PATH = _ROOT / "submissions" / "archive" / "v0.9_v8_cleaned.csv"
...
def main() -> None:
    ...
    # === Outlier identification + weighting ===
    print("\nIdentifying outliers + computing training weights...")
    impossible = identify_impossible_rows(df_train)
    weights_train = compute_training_weights(df_train, downweight_2022=0.7)
    n_impossible = int(impossible.sum())
    n_2022_downweighted = int(((weights_train == 0.7)).sum())
    print(f"  Impossible rows (weight 0): {n_impossible}")
    print(f"  2022 rows (weight 0.7): {n_2022_downweighted}")
    print(f"  Effective train size: {weights_train.sum():.0f} rows")

    # === Build features (on ALL rows incl. impossible — power curve filters them) ===
    print("\nBuilding features...")
    # Save weights indexed by original row before any sort.
    df_train["_weight"] = weights_train
    df_train = build_features(df_train, sort_by_time=False)

    # Curtailment rate by hour (computed on CLEAN training data only).
    clean_mask = df_train["_weight"] > 0
    curtail_rates = compute_curtail_rate_by_hour(df_train[clean_mask])
    print(f"  Curtailment rate by hour (min/max): {curtail_rates.min():.3f} / {curtail_rates.max():.3f}")

    # Add curtailment features to train and valid.
    ...
    for fold in folds:
        ...
        # Power curve fit ONLY on clean rows in this fold's training.
        fold_train = df_train.iloc[train_idx]
        clean = fold_train[fold_train["_weight"] > 0]
        pc_sector = fit_sector_isotonic(clean, n_sectors=8)
        pc_global = IsotonicPowerCurve().fit(clean["v_eff"], clean[TARGET_COL])
        ...
        booster = _train_single(X_tr, y_tr, w_tr, X_va, y_va, feat_cols, 42, config_base)
        ...
        print(f"  {fold.name}: nMAE = {nmae:.4f} %  (best_iter={booster.best_iteration}, n_features={len(feat_cols)})")
    ...
    df_tr = _add_power_curve_features(fold_train, pc_sector, pc_global)
    df_va = _add_power_curve_features(df_train.iloc[val_idx], pc_sector, pc_global)
    feat_cols = [c for c in feature_columns(df_tr) if c != "_weight"]
    ...
    clean_full = df_train[df_train["_weight"] > 0]
    pc_sector_full = fit_sector_isotonic(clean_full, n_sectors=8)
    pc_global_full = IsotonicPowerCurve().fit(clean_full["v_eff"], clean_full[TARGET_COL])
    df_train_full = _add_power_curve_features(df_train, pc_sector_full, pc_global_full)
    feat_cols = [c for c in feature_columns(df_train_full) if c != "_weight"]
    print(f"  Features: {len(feat_cols)}")

    X_full = df_train_full[feat_cols].to_numpy(dtype=np.float32)
    y_full = df_train_full[TARGET_COL].to_numpy(dtype=np.float32)
    w_full = df_train_full["_weight"].to_numpy(dtype=np.float32)

    n_rounds = max(int(np.median(best_iters) * 1.2), 1000)
    print(f"  n_rounds per seed: {n_rounds}")
    boosters_full = [
        _train_full(X_full, y_full, w_full, feat_cols, s, n_rounds, config_base) for s in SEEDS
    ]
