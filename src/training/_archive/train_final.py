"""FINAL: best single configuration discovered through experiments v1-v11.

Configuration:
- Physics-aware features (REWS, v_eff, Hellmann, air density).
- Per-sector isotonic power curve fit ONLY on clean rows.
- Full training set fed to LGBM (power curve handles outliers).
- v7-tuned hyperparameters (Optuna on Fold-4 + Fold-5).
- 5-seed LGBM ensemble.
- Physics-aware post-processing (cut-in/cut-out).
- No bias calibration (proven to hurt across folds).
- No season weighting (proven to hurt).
- No curtailment features (proven to hurt).

Expected Fold-5 nMAE: ~8.73%
...
SUBMISSION_PATH = _ROOT / "submissions" / "archive" / "v1.2_final.csv"
...
def main() -> None:
    ...
    print("Identifying outliers for power curve filtering...")
    impossible = identify_impossible_rows(df_train)
    df_train["_is_impossible"] = impossible.values
    n_imp = int(impossible.sum())
    print(f"  {n_imp} outliers excluded from power curve fit (kept in LGBM training)")
    ...
    # === Fold-5 multi-seed ensemble (final Q1 surrogate benchmark) ===
    ...
    for i, b in enumerate(boosters_full):
        b.save_model(str(MODEL_DIR / f"lgbm_final_seed{SEEDS[i]}.txt"))
