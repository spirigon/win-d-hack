"""V6: v5 feature-selected + manufacturer power curve features.

Adds datasheet-based theoretical power (Siemens Gamesa SG 3.4-132) as
physics-informed features on top of ERA5. The datasheet features encode:
- Per-turbine rated curve at multiple air densities (bilinear interp)
- Farm-level theoretical power (× active_turbines)
- Residual from datasheet (captures wake losses and other deviations)

Then apply K=70 feature selection.

Usage:
    python -m src.training.train_v6_datasheet
"""
...
SUBMISSION_PATH = _ROOT / "submissions" / "archive" / "v6.0_datasheet.csv"
...
def _post_process(preds, v_eff):
    ...
    # NOTE: at ws > 25, the turbine still produces ~63 MW farm-level per datasheet,
    # so we DON'T clamp to 5 MW anymore. Just clip to capacity.
...
def main():
    ...
    # Show where datasheet features rank.
    print("\n  Datasheet features in importance:")
    for rank, (name, score) in enumerate(feat_imp, 1):
        if "ds_" in name:
            print(f"    rank {rank:3d}: {name:45s} {score:10.1f}")

    # Ablation over K.
    print("\n  === Ablation (5 seeds each) ===")
    ks = [60, 70, 80, 90, 100, 110, len(feat_cols_all)]
    results = {}
    best_nmae = 999
    best_k = 0
    for k in ks:
        ...
        results[k] = nmae
        print(f"    K={k:3d}: Fold-5 nMAE = {nmae:.4f} %")
        if nmae < best_nmae:
            best_nmae = nmae
            best_k = k
    ...
    # Full run with best K + 10 seeds.
    top_k_final = [n for n, _ in feat_imp[:best_k]]
    print(f"\n  === Final with K={best_k}, 10 seeds ===")
    X_tr = df_tr[top_k_final].to_numpy(dtype=np.float32)
    X_va = df_va[top_k_final].to_numpy(dtype=np.float32)

    preds_all = []
    best_iters = []
    for seed in SEEDS:
        b = _train_single(X_tr, y_tr, X_va, y_va, top_k_final, seed, config_base)
    ...
    ens = np.mean(preds_all, axis=0)
    print(f"\n    Ensemble Fold-5 nMAE: {normalized_mae(y_va, ens):.4f}%")

    # Full-fit on all train.
    print("\n  Full-fit...")
    ...
    boosters_full = [_train_full(X_full, y_full, top_k_final, s, n_rounds, config_base) for s in SEEDS]
    ...
    # Blend with v5.0 and v2.0.
    v50 = pd.read_csv(_ROOT / "submissions" / "archive" / "v5.0_fs70.csv", header=None)[0].to_numpy()
    v20 = pd.read_csv(_ROOT / "submissions" / "archive" / "v2.0_era5.csv", header=None)[0].to_numpy()
    blend3 = np.clip((po + v50 + v20) / 3.0, 0, CAPACITY_MW)
    write_submission(blend3, _ROOT / "submissions" / "archive" / "v6.1_blend3.csv", expected_rows=len(df_valid))
    blend2 = np.clip((po + v50) / 2.0, 0, CAPACITY_MW)
    write_submission(blend2, _ROOT / "submissions" / "archive" / "v6.2_v5v6_blend.csv", expected_rows=len(df_valid))

    MODEL_DIR.mkdir(parents=True, exist_ok=True)
    for i, b in enumerate(boosters_full):
        b.save_model(str(MODEL_DIR / f"lgbm_v6_seed{SEEDS[i]}.txt"))
    print("\nDone.")
