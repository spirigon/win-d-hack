"""Feature selection on PURE v2 ERA5 setup (the one that scored 7.924).

The v2 setup has 106 features (basic ERA5, no advanced). Test K in
[30, 40, 50, 60, 70, 80, 90, 100, 106] to find the sweet spot.

Usage:
    python -m src.training.train_v2_feature_select
"""
...
def main():
    ...
    for name, score in feat_imp[:20]:
        print(f"    {name:45s} {score:10.1f}")

    # === Ablation over K ===
    print("\n=== Fold-5 ensemble nMAE by K (top-K features) ===")
    results = {}
    ks = [20, 25, 30, 35, 40, 50, 60, 70, 80, 100, len(feat_cols_all)]
    for k in ks:
    ...
    print(f"\n  Best K = {best_k}  (nMAE = {results[best_k]:.4f} %)")
