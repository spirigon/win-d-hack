"""V92: V75 + hub-height (85 m) consensus shear + seasonal + 5 seeds + GPU, CF/MW blend.

Key insight: turbine hub is at 85 m, but model uses wind_speed_120m throughout.
Using 120 m overestimates theoretical power by ~17% on average (v³ scaling,
mean shear ratio v_85/v_120 ≈ 0.944).  The shear correction varies with stability,
so explicit 85 m features are needed.

New features vs V75 (5 total, lean set):
  shear_exp_consensus  : mean shear exponent across (10→80, 10→120, 80→120) pairs
  wind_speed_85m_pl    : v_120 * (85/120)^m_consensus  — correct hub speed
  ds_85m_pl_farm_mw    : theoretical power at 85 m hub speed
  ds_85m_pl_ratio      : capacity factor at hub speed
  ds_85m_vs_120m_mw    : shear correction magnitude (always negative)
  + month_sin/cos_x_veff, month_x_ws120  — seasonal (confirmed +0.016pp)

K=80, 5 seeds, GPU-accelerated (RTX 3060 Ti).

Outputs:
    data/processed/v92_oof.parquet
    submissions/archive/v92.0_hub_height.csv
"""
...
OOF_PATH    = _ROOT / "data" / "processed" / "v92_oof.parquet"
OUTPUT_PATH = _ROOT / "submissions" / "archive" / "v92.0_hub_height.csv"
...
def main() -> None:
    ...
    print("=" * 72)
    print(f"V92: V75 + hub-height 85m + seasonal  ({len(SEEDS)} seeds, K={K})")
    print("=" * 72)
