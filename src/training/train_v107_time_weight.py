"""V107: V99 pipeline + exponential temporal weighting of training samples.

Hypothesis: turbine performance drifts over time (aging, blade wear, seasonal
calibration shifts). Training samples closer in time to each fold's validation
period are more representative of current behavior.

Implementation: within each fold's training set, multiply sample weights by
exp(-LAMBDA * delta_t / max_delta_t) where delta_t is the time gap from the
fold's train_end to that sample. Higher LAMBDA = more aggressive discounting
of older data.

LAMBDA=0.5 means data 1 year before fold's train_end gets 0.607× weight.
LAMBDA=1.0 means data 1 year before gets 0.368× weight.

We use LAMBDA=0.75 as a moderate discount (between the two extremes).

All other design choices identical to V99: corr dedup, K=90, 5 seeds, CF+MW blend.

Outputs:
    data/processed/v107_oof.parquet
    submissions/archive/v107.0_time_weight.csv
"""
...
OOF_PATH    = _ROOT / "data" / "processed" / "v107_oof.parquet"
OUTPUT_PATH = _ROOT / "submissions" / "archive" / "v107.0_time_weight.csv"

MARCH_WEIGHT   = 3.0
BLEND_WEIGHT_MW = 0.0  # CF-only proven optimal by blend_weight_optimizer
TIME_LAMBDA    = 0.75   # exponential decay rate for temporal weighting
SEEDS          = SEEDS_5
K              = 90
...
def _time_weight(ts: np.ndarray, train_end_ts: pd.Timestamp, lam: float) -> np.ndarray:
    """Exponential temporal weight: samples closer to train_end get more weight.

    delta_t_hours = (train_end - t) / 1e9 / 3600  (ns → hours)
    max_delta = max of all delta_t values
    weight = exp(-lam * delta_t / max_delta)
    """
    ts_ns = ts.astype("int64")
    train_end_ns = int(train_end_ts.value)
    delta_t = (train_end_ns - ts_ns) / 1e9 / 3600.0  # hours before fold train_end
    delta_t = np.clip(delta_t, 0.0, None)
    max_delta = float(delta_t.max())
    if max_delta < 1.0:
        return np.ones(len(ts), dtype=np.float32)
    return np.exp(-lam * delta_t / max_delta).astype(np.float32)
...
def main() -> None:
    ...
    print("=" * 72)
    print(f"V107: time-weighted training (lambda={TIME_LAMBDA}) + corr dedup  "
          f"({len(SEEDS)} seeds, K={K})")
    print("=" * 72)
    ...
    # Base sample weights (curtailment + March boost) — time weights added per-fold
    base_sw = (~is_impossible_full.to_numpy()[: len(df_train_full_pre)]).astype(np.float32)
    march_mask = df_train_full_pre[TIMESTAMP_COL].dt.month == 3
    base_sw[march_mask] *= MARCH_WEIGHT
    train_ts_all = df_train_full_pre[TIMESTAMP_COL].to_numpy()
    n_curtail = int(impossible.sum())
    n_march   = int(march_mask.sum())
    n_zero    = int((base_sw == 0).sum())
    ...
    print(f"  Time decay: lambda={TIME_LAMBDA}")
    ...
    # Probe with time-weighted fold5 samples
    ts_probe = df_t_[TIMESTAMP_COL].to_numpy()
    tw_probe = _time_weight(ts_probe, fold5.train_end, TIME_LAMBDA)
    sw_probe = base_sw[train_idx] * tw_probe
    ...
    print(f"\n[3/4] Training CV-bag (folds {[i + 1 for i in FOLD_IDS]}, "
          f"CF+MW blend, seeds={SEEDS}, time_lambda={TIME_LAMBDA})...")
    ...
    for target_mode in ("cf", "mw"):
        ...
        for fold_idx in FOLD_IDS:
            ...
            # Per-fold time weights applied to base sample weights
            ts_fold = df_tr[TIMESTAMP_COL].to_numpy()
            tw_fold = _time_weight(ts_fold, fold.train_end, TIME_LAMBDA)
            sw      = base_sw[train_idx] * tw_fold
