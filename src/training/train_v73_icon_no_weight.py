"""V68: V64 (March 3x only) + ICON-EU NWP ensemble features.

V67 used V66's weighting (Jan/Feb x2.0 + Mar x3.0), which hurt fold-5.
V68 restores V64's winning recipe: March 3x ONLY, no Jan/Feb weighting.
ICON-EU features are layered on top of that cleaner baseline.

ICON-EU provides genuine NWP diversity (corr=0.91 with ERA5, std=1.41 m/s).
Coverage: 2023-01-01 onward; 2022 rows have NaN — LightGBM handles natively.

Key signal validated from Fold-5 OOF:
    When ICON < ERA5 by >2 m/s (117 rows): bias = -4.20 MW
    => ICON-ERA5 delta is a useful corrective signal for ERA5 over-estimation.

Outputs:
    data/processed/v73_oof.parquet
    data/processed/v73_test.parquet
    submissions/archive/v73.0_icon_no_weight.csv

Usage:
    python -m src.training.train_v73_icon_no_weight --seeds 3
    python -m src.training.train_v73_icon_no_weight --seeds 5
"""
...
OOF_PATH    = _ROOT / "data" / "processed" / "v73_oof.parquet"
TEST_PATH   = _ROOT / "data" / "processed" / "v73_test.parquet"
OUTPUT_PATH = _ROOT / "submissions" / "archive" / "v73.0_icon_no_weight.csv"

# No MARCH_WEIGHT — v73 uses no weighting
...
def main() -> None:
    ...
    print("=" * 72)
    print(f"V73: ICON-EU only, no March weighting  ({len(seeds)} seeds)")
    ...
    # No March weighting — test if March 3x overfits to 2025 curtailment

    print("  No sample weighting (v73 tests ICON without March 3x)")
