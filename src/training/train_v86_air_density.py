"""V86: V75 features + air density features + CF/MW 50/50 blend.

Air density rho = P / (R * T).  Jan-Mar (test period): ~1.30 kg/m³ vs
standard 1.225 → +5.5% power.  Model underestimates in steep zone (7-10 m/s)
by 1.42 MW; seasonal density gap likely contributes.

New features vs V75:
  - air_density_norm  : rho / 1.225  (~1.055 in winter)
  - v120_density      : wind_speed_120m * rho_norm^(1/3)
  - v_eff_density     : v_eff * rho_norm^(1/3)
  - wpd_density       : wind_speed_120m^3 * rho_norm
  - gfs_ws100m_density, ecmwf_ws100m_density

Everything else identical to V75:
  - CF + MW dual-target, BLEND_WEIGHT_MW=0.5
  - Walk-forward availability features
  - Curtailment / impossible-row exclusion
  - March × 3.0 weighting
  - Feature selection probe K=80
  - Folds [3, 4, 5], 3 seeds

Outputs:
    data/processed/v86_oof.parquet
    submissions/archive/v86.0_air_density.csv
"""
...
OOF_PATH    = _ROOT / "data" / "processed" / "v86_oof.parquet"
OUTPUT_PATH = _ROOT / "submissions" / "archive" / "v86.0_air_density.csv"
...
def main() -> None:
    ...
    print("=" * 72)
    print(f"V86: V75 features + air density features  ({len(SEEDS)} seeds, K={K})")
    print("=" * 72)
    ...
    # --- Air density features (new in v86) ---
    ...
    print(f"  Feature groups: nwp={len(nwp_cols)} nasa={len(nasa_cols)} "
          f"mnwp={len(mnwp_cols)} adv={len(adv_cols)} density={len(dens_cols)}")
    ...
    n_curtail = int((~is_impossible_full.to_numpy()[: len(df_train_full_pre)]).sum() == 0 or True) - 1
    ...
    all_extra_cols = list(dict.fromkeys(nwp_cols + nasa_cols + mnwp_cols + adv_cols + dens_cols))
    feat_cols_all = [
