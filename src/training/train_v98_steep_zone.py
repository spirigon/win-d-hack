"""V98: V94 + steep zone sample upweighting (7–10 m/s transition zone).

OOF analysis of v94 shows a systematic bias in the power curve transition zone:
  steep_7-10 m/s: nMAE=13.19%  bias=−1.47 MW (model underestimates)
  mid_10-14 m/s:  nMAE=11.07%  bias=+2.29 MW (model overestimates)

The model underestimates in the steepest part of the power curve because small
NWP wind speed errors cause large power errors there.  Upweighting the 7-10 m/s
training samples forces the model to be more accurate in this zone.

Change vs v94:
  sample_weight_full[steep_mask] *= 2.0  for ws in [7, 10.5) m/s

K=90, 5 seeds, CPU.

Outputs:
    data/processed/v98_oof.parquet
    submissions/archive/v98.0_steep_zone.csv
"""
...
OOF_PATH    = _ROOT / "data" / "processed" / "v98_oof.parquet"
OUTPUT_PATH = _ROOT / "submissions" / "archive" / "v98.0_steep_zone.csv"

MARCH_WEIGHT    = 3.0
STEEP_WEIGHT    = 2.0   # multiply sample weight for ws 7-10.5 m/s samples
STEEP_WS_LOW    = 7.0
STEEP_WS_HIGH   = 10.5
SEEDS = SEEDS_5
K = 90
...
def main() -> None:
    ...
    print("=" * 72)
    print(f"V98: steep zone x{STEEP_WEIGHT} upweight  ({len(SEEDS)} seeds, K={K})")
    print("=" * 72)
    ...
    # Upweight steep zone (7-10.5 m/s): power curve transition has highest error
    ws_train = df_train_full_pre["wind_speed_120m"].to_numpy()
    steep_mask = (ws_train >= STEEP_WS_LOW) & (ws_train < STEEP_WS_HIGH)
    sample_weight_full[steep_mask] *= STEEP_WEIGHT
    ...
    print(f"  Steep zone x{STEEP_WEIGHT} ({STEEP_WS_LOW}-{STEEP_WS_HIGH} m/s): "
          f"{n_steep} ({100*n_steep/len(df_train_full_pre):.1f}%)")
    ...
    all_extra_cols = list(dict.fromkeys(
        nwp_cols + nasa_cols + mnwp_cols + adv_cols + hub_cols + seas_cols
    ))
    feat_cols_all = [
    ...
    for label, cols in [("hub_height", hub_cols), ("seasonal", seas_cols)]:
