"""V69: V68 with neutral imputation of 2022 ICON-EU NaN rows.

V68 (v64 + ICON-EU, no NaN imputation) was worse than v64 — the NaN-branch
splits on 2022 rows (~27% of fold-5 training) consumed model capacity.

V69 fixes this: 2022 rows get ICON = ERA5 (delta=0, ratio=1) so LightGBM
never fires a NaN branch. This tests whether ICON-EU's 2023+ diversity
can be isolated without the NaN-branch overhead penalty.

ICON-EU provides genuine NWP diversity (corr=0.91 with ERA5, std=1.41 m/s).
Key signal: When ICON < ERA5 by >2 m/s (117 Fold-5 rows): bias = -4.20 MW.

Outputs:
    data/processed/v69_oof.parquet
    data/processed/v69_test.parquet
    submissions/archive/v69.0_march_icon.csv

Usage:
    python -m src.training.train_v69_march_icon_imputed --seeds 3
    python -m src.training.train_v69_march_icon_imputed --seeds 5
"""
...
OOF_PATH    = _ROOT / "data" / "processed" / "v69_oof.parquet"
TEST_PATH   = _ROOT / "data" / "processed" / "v69_test.parquet"
OUTPUT_PATH = _ROOT / "submissions" / "archive" / "v69.0_march_icon.csv"
...
def main() -> None:
    ...
    print("=" * 72)
    print(f"V69: V64 (March 3x only) + ICON-EU NWP features  ({len(seeds)} seeds)")
    ...
    combined = merge_nwp_ensemble(combined, impute_missing=True)
