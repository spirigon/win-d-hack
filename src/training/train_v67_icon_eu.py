"""V67: V66 + ICON-EU wind features as independent NWP ensemble.

Three improvements over V66:
1. ICON-EU wind speed at 100m — independent NWP estimate (corr=0.91 with ERA5)
2. ICON-ERA5 wind speed delta — model spread proxy for uncertainty
3. Richardson number from T80/T120 (already in extras since V66)

ICON-EU coverage: 2023-01-01 onward. 2022 rows have icon_ws100m=NaN;
LightGBM handles these via NaN-branch splitting — effectively treats 2022
as a separate regime and still benefits from 2023-2025 ICON diversity.

Key validation result from fold-5 OOF analysis:
    When ICON predicts <2 m/s below ERA5 (117 rows): bias=-4.20 MW
    -> ICON lower than ERA5 = ERA5 over-estimates -> correct downward

Expected improvement: 0.10-0.20 pp over V66 on fold-5 nMAE.

Outputs:
    data/processed/v67_oof.parquet
    data/processed/v67_test.parquet
    submissions/archive/v67.0_icon_eu.csv

Usage:
    python -m src.training.train_v67_icon_eu --seeds 5
    python -m src.training.train_v67_icon_eu --seeds 3
"""
...
OOF_PATH    = _ROOT / "data" / "processed" / "v67_oof.parquet"
TEST_PATH   = _ROOT / "data" / "processed" / "v67_test.parquet"
OUTPUT_PATH = _ROOT / "submissions" / "archive" / "v67.0_icon_eu.csv"
...
def main() -> None:
    ...
    print("=" * 72)
    print(f"V67: V66 + ICON-EU NWP ensemble features  ({len(seeds)} seeds)")
    print(f"     Winter weighting: Jan/Feb x{JAN_FEB_WEIGHT}, Mar x{MARCH_WEIGHT}")
    print("=" * 72)
    ...
    # ICON-EU: independent NWP wind at 100m (NaN for 2022 — LGBM handles natively)
    combined = merge_nwp_ensemble(combined)

    nwp_cols = nwp_ensemble_columns(combined)
    print(f"  ICON-EU / NWP features added: {len(nwp_cols)} — {nwp_cols}")
    icon_valid = combined["icon_ws100m"].notna().mean() if "icon_ws100m" in combined.columns else 0
    print(f"  icon_ws100m coverage: {icon_valid:.1%} of rows")
    ...
    train_month = df_train_full_pre[TIMESTAMP_COL].dt.month
    jan_feb_mask = train_month.isin([1, 2])
    march_mask   = train_month == 3
    sample_weight_full[jan_feb_mask] *= JAN_FEB_WEIGHT
    sample_weight_full[march_mask]   *= MARCH_WEIGHT

    n_janfeb = int(jan_feb_mask.sum())
    n_march  = int(march_mask.sum())
    total    = len(df_train_full_pre)
    print(f"  Jan+Feb rows x{JAN_FEB_WEIGHT}: {n_janfeb} ({100*n_janfeb/total:.1f}%)")
    ...
    # Explicitly include NWP ensemble columns even if feature_columns misses them
    for col in nwp_cols:
    ...
    print(f"  Feature pool: {len(feat_cols_all)}  "
          f"({n_era5v2} era5v2_*, {n_nwp} nwp_ensemble_*, {n_avail} avail_*)")
    ...
    # Report which NWP features made the top K
    nwp_in_topk = [c for c in top_k if c in nwp_cols]
    print(f"  NWP ensemble features in top-{K}: {nwp_in_topk}")
