"""V80: V76 without power lags (K=120, turbulence, curtailment, March-only weight).

The v78 experiment showed that at K=80, turbulence features are squeezed out
(only 2 make top-80).  But the steep zone (7-10 m/s) has r=0.68, the lowest
correlation of any wind speed bin.  Turbulence intensity and ensemble rank
features specifically address variance in this zone.

At K=120, v76 had 10 turbulence features selected, addressing:
    - icon_ti_10m, ti_multi_std, ti_multi_max  (turbulence intensity proxies)
    - nwp_vs_ens_median_sq                     (ensemble rank deviation)
    - wpd_ens_mean, era5v2_v_eff_120m           (WPD / effective wind)
    - ecmwf_ti_10m                              (ECMWF TI)

This version is "V76 without lags":
    - K = 120 (allows turbulence features to contribute)
    - All V76 features EXCEPT power lag features
    - No seasonal month weights beyond March x3
    - Equal fold blend (1/3 each) -- no recency adjustment
    - Curtailment weights (3.2% of rows zeroed)
    - CF-only, 5 seeds

Baseline: V75 = 7.43% LB (K=80, no turbulence, no lags)
Goal: improve on 7.43% via K=120 + turbulence in steep zone

Outputs:
    data/processed/v80_oof.parquet
    data/processed/v80_test.parquet
    submissions/archive/v80.0_k120_turb.csv

Usage:
    python -m src.training.train_v80_k120_turb --seeds 3
    python -m src.training.train_v80_k120_turb --seeds 5
"""
...
OOF_PATH    = _ROOT / "data" / "processed" / "v80_oof.parquet"
TEST_PATH   = _ROOT / "data" / "processed" / "v80_test.parquet"
OUTPUT_PATH = _ROOT / "submissions" / "archive" / "v80.0_k120_turb.csv"
...
def main() -> None:
    ...
    print("=" * 72)
    print(f"V80: V76 - lags, K=120, turbulence, March x{MARCH_WEIGHT}  ({len(seeds)} seeds)")
    print("=" * 72)
