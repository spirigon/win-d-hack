"""V96: V94 (hub+seasonal, K=90) + air density correction + diurnal cycle.

Hypothesis: Two untapped physics signals:
  1. Air density: P ∝ ρ·v³, actual ρ varies ±5% seasonally (sp, t2m → ρ=sp/287T).
     Standard power curves assume ρ₀=1.225 kg/m³; we correct this explicitly.
  2. Diurnal cycle: nocturnal low-level jets → hour-dependent wind shear profile,
     distinct from the seasonal cycle already captured by month_sin/cos.

New feature groups (vs v94):
  density (9):  air_density, density_ratio, kinetic_power_120m, ds_120m_farm_mw_rho, ...
  diurnal (10): hour_sin, hour_cos, hour_sin/cos_x_ws120, is_day, daytime_ws, ...

K=90, 5 seeds, CPU.

Outputs:
    data/processed/v96_oof.parquet
    submissions/archive/v96.0_density_diurnal.csv
"""
...
from src.features.density_features import add_density_features, density_columns
from src.features.diurnal_features import add_diurnal_features, diurnal_columns
...
OOF_PATH    = _ROOT / "data" / "processed" / "v96_oof.parquet"
OUTPUT_PATH = _ROOT / "submissions" / "archive" / "v96.0_density_diurnal.csv"

MARCH_WEIGHT = 3.0
SEEDS = SEEDS_5
K = 90  # wider K, clean pool + density + diurnal
...
def main() -> None:
    ...
    print("=" * 72)
    print(f"V96: hub+seasonal+density+diurnal  ({len(SEEDS)} seeds, K={K})")
    print("=" * 72)
    ...
    combined = add_density_features(combined)      # air density correction (sp, t2m → ρ)
    combined = add_diurnal_features(combined)      # hour-of-day harmonics + interactions
    ...
    den_cols  = density_columns(combined)
    diu_cols  = diurnal_columns(combined)
    ...
    print(f"  Feature groups: nwp={len(nwp_cols)} nasa={len(nasa_cols)} "
          ...
          f"density={len(den_cols)} diurnal={len(diu_cols)}")
    ...
    all_extra_cols = list(dict.fromkeys(
        nwp_cols + nasa_cols + mnwp_cols + adv_cols + hub_cols + seas_cols + den_cols + diu_cols
    ))
    feat_cols_all = [
    ...
    for label, cols in [
        ...
        ("density", den_cols), ("diurnal", diu_cols),
    ]:
