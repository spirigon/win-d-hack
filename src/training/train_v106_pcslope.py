"""V106: V99 pipeline + power curve slope and steep-zone features.

Power curve slope dP/dV tells the model how sensitive predicted power is
to wind speed error at the current operating point.  In the steep zone
(7-10.5 m/s) the slope is 400-700 kW/m/s — much higher than below cut-in
or near rated power.  Making this explicit helps the model weight its
uncertainty appropriately by wind speed regime.

New features added (computed from the datasheet power curve):
  ds_pc_slope_mw_per_ms   : farm-level power curve slope at current ws
  ds_steep_zone           : binary flag for ws in [7, 10.5] m/s
  ds_slope_x_roll6h       : slope × era5_ws100_roll_mean_6h interaction
  ds_slope_x_icing        : slope × icing_severity interaction

All other design choices identical to V99 (corr dedup, K=90, 5 seeds, CF+MW blend).

Outputs:
    data/processed/v106_oof.parquet
    submissions/archive/v106.0_pcslope.csv
"""
...
from src.features.datasheet_power_curve import (
    WS_GRID, add_datasheet_power_features, per_turbine_power_kw,
)
...
OOF_PATH    = _ROOT / "data" / "processed" / "v106_oof.parquet"
OUTPUT_PATH = _ROOT / "submissions" / "archive" / "v106.0_pcslope.csv"
...
def _add_pc_slope_features(df: pd.DataFrame) -> pd.DataFrame:
    """Add power curve slope and steep-zone interaction features."""
    df = df.copy()
    ws = df["wind_speed_120m"].to_numpy()
    density = df["air_density"].to_numpy()
    active = df["active_turbines"].to_numpy()

    # Numerical derivative: central difference with h=0.5 m/s
    eps = 0.5
    slope_kw = per_turbine_power_kw(ws + eps, density) - per_turbine_power_kw(ws - eps, density)
    # slope_kw = kW per 1 m/s change; convert to farm-level MW per m/s
    df["ds_pc_slope_mw_per_ms"] = np.clip(slope_kw * active / 1000.0, 0.0, None) / (2 * eps)

    # Steep zone flag (where power curve slope is steepest, 7-10.5 m/s)
    df["ds_steep_zone"] = ((ws >= 7.0) & (ws <= 10.5)).astype(np.float32)

    # Interaction: slope × recent wind history (high slope + strong wind trend = high uncertainty)
    if "era5_ws100_roll_mean_6h" in df.columns:
        df["ds_slope_x_roll6h"] = df["ds_pc_slope_mw_per_ms"] * df["era5_ws100_roll_mean_6h"]

    # Interaction: slope × icing severity (steep zone + icing = highest error)
    if "icing_severity" in df.columns:
        df["ds_slope_x_icing"] = df["ds_pc_slope_mw_per_ms"] * df["icing_severity"]
...
def main() -> None:
    ...
    print("=" * 72)
    print(f"V106: PC slope features + corr dedup  ({len(SEEDS)} seeds, K={K})")
    print("=" * 72)
    ...
    combined = _add_pc_slope_features(combined)  # NEW
    ...
    # Add PC slope features explicitly
    pc_slope_cols = ["ds_pc_slope_mw_per_ms", "ds_steep_zone", "ds_slope_x_roll6h", "ds_slope_x_icing"]
    for col in pc_slope_cols:
        if col in df_t_.columns and col not in feat_cols_all:
            feat_cols_all.append(col)

    # Pass 1: byte-identical dedup
    ...
    # Check which PC slope features survived dedup
    pc_survived = [c for c in pc_slope_cols if c in feat_cols_all]
    print(f"  PC slope features in pool: {pc_survived}")
    ...
    # Report PC slope feature ranks
    for col in pc_slope_cols:
        rank = next((i + 1 for i, (n, _) in enumerate(feat_imp) if n == col), None)
        if rank is not None:
            in_top = col in top_k
            gain = feat_imp[rank - 1][1]
            print(f"  {col}: rank={rank} gain={gain:.0f} {'[in top-K]' if in_top else '[excluded]'}")
