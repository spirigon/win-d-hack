"""V89: V75 features, all 5 folds in the CV-bag, CF/MW 50/50 blend.

Hypothesis: test predictions averaged over 5 fold-models (folds 1-5) are more
robust than the current 3-fold bag.  Fold 1 (Q1-2023) and Fold 2 (Q4-2023)
add earlier seasonal data; Fold 1 is especially relevant since it's the same
calendar quarter (Jan-Mar) as the Jan-Mar 2026 test period.

No new features vs V75 — pure fold-diversity experiment.

Outputs:
    data/processed/v89_oof.parquet
    submissions/archive/v89.0_5fold.csv
"""
...
OOF_PATH    = _ROOT / "data" / "processed" / "v89_oof.parquet"
OUTPUT_PATH = _ROOT / "submissions" / "archive" / "v89.0_5fold.csv"

MARCH_WEIGHT = 3.0
SEEDS = SEEDS_3
ALL_FOLD_IDS = [0, 1, 2, 3, 4]   # All 5 folds (1-indexed: 1,2,3,4,5)
PROBE_FOLD_ID = 4                  # Feature selection still uses Fold-5 (0-indexed 4)
...
def main() -> None:
    ...
    print("=" * 72)
    print(f"V89: V75 features, 5-fold bag  ({len(SEEDS)} seeds, K={K})")
    print(f"     Folds: {[i+1 for i in ALL_FOLD_IDS]}  (vs v75: [3,4,5])")
    print("=" * 72)
    ...
    print(f"\n[3/4] Training CV-bag (folds {[i + 1 for i in ALL_FOLD_IDS]}, "
    ...
    # Average test predictions over all 5 folds
    avg_cf = np.mean([test_cf_per_fold[i] for i in ALL_FOLD_IDS], axis=0)
    avg_mw = np.mean([test_mw_per_fold[i] for i in ALL_FOLD_IDS], axis=0)
