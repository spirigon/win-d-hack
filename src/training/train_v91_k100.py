"""V91: V75 + all new features, K=100, 5 seeds, CF/MW blend.

Hypothesis: smear and seasonal features ranked 81-100 in the K=80 probe.
Raising K to 100 gives even more room for borderline-useful features.

New vs V75:
  - air density features   (6 cols) — likely redundant, probe will confirm
  - smeared power features (12 cols) — P(v±δ)/2 at delta=[1,2,3,4]
  - seasonal interactions  (6 cols) — month × wind speed

Key changes vs V87:
  - K: 80 → 100  (20 extra features allowed)
  - Seeds: 3 → 5  (matches v75, eliminates 0.04pp seed-count penalty)

Outputs:
    data/processed/v91_oof.parquet
    submissions/archive/v91.0_k100.csv
"""
...
from src.features.smeared_power_features import (
    add_smeared_power_features, smeared_power_columns, DELTA_V_LIST,
)
...
from src.training.train_v32_era5v2 import (
    LGBM_PARAMS, BLEND_WEIGHT_MW, FOLD_IDS, SEEDS_5,
...
OOF_PATH    = _ROOT / "data" / "processed" / "v91_oof.parquet"
OUTPUT_PATH = _ROOT / "submissions" / "archive" / "v91.0_k100.csv"
...
def main() -> None:
    ...
    print("=" * 72)
    print(f"V91: V75 + all new features, K={K}, {len(SEEDS)} seeds, CF/MW blend")
    print(f"     Smear deltas: {DELTA_V_LIST} m/s | March x{MARCH_WEIGHT}")
    print("=" * 72)
    ...
    print(f"  Feature groups: nwp={len(nwp_cols)} nasa={len(nasa_cols)} "
          ...
          f"density={len(dens_cols)} smear={len(smear_cols)} seasonal={len(seas_cols)}")
    ...
    all_extra_cols = list(dict.fromkeys(
        nwp_cols + nasa_cols + mnwp_cols + adv_cols + dens_cols + smear_cols + seas_cols
    ))
    feat_cols_all = [
    ...
    # Show what made it in at ranks 81-90 (new territory vs K=80)
    new_slots = [n for n, _ in feat_imp[80:K]]
    print(f"  Top-{K} selected  (top-3: {[n for n, _ in feat_imp[:3]]})")
    print(f"  Features at ranks 81-{K} (new territory): {new_slots}")
    for label, cols in [("smear", smear_cols), ("density", dens_cols), ("seasonal", seas_cols)]:
